 ...
 * {@link SpringServletContainerInitializer}, which itself is bootstrapped automatically
 * by any Servlet 3.0 container. See {@linkplain SpringServletContainerInitializer its
 * Javadoc} for details on this bootstrapping mechanism.
 *
 * <h2>Example</h2>
 * <h3>The traditional, XML-based approach</h3> Most Spring users building a web
 * application will need to register Spring's {@code
 * DispatcherServlet}. For reference, in WEB-INF/web.xml, this would typically be done as
 * follows: <pre class="code">
 * &lt;servlet&gt;
 ...
 * &lt;/servlet-mapping&gt;</pre>
 *
 * <h3>The code-based approach with {@code WebApplicationInitializer}</h3> Here is the
 * equivalent {@code DispatcherServlet} registration logic,
 * {@code WebApplicationInitializer}-style: <pre class="code">
 ...
 * {@link org.springframework.web.servlet.support.AbstractDispatcherServletInitializer}.
 *
 ...
 *
 * <h2>A 100% code-based approach to configuration</h2> In the example above,
 * {@code WEB-INF/web.xml} was successfully replaced with code in the form of a
 * {@code WebApplicationInitializer}, but the actual {@code dispatcher-config.xml} Spring
 * configuration remained XML-based. {@code WebApplicationInitializer} is a perfect fit
 * for use with Spring's code-based {@code @Configuration} classes.
 * See @{@link org.springframework.context.annotation.Configuration Configuration} Javadoc
 * for complete details, but the following example demonstrates refactoring to use
 * Spring's
 ...
 * context and registration of the {@code ContextLoaderListener}: <pre class="code">
 * public class MyWebAppInitializer implements WebApplicationInitializer {
 *
 *    &#064;Override
 *    public void onStartup(ServletContext container) {
 ...
 * interface. If so, the initializers will be ordered prior to invocation. This provides a
 * mechanism for users to ensure the order in which servlet container initialization
 * occurs. Use of this feature is expected to be rare, as typical applications will likely
 * centralize all container initialization within a single
