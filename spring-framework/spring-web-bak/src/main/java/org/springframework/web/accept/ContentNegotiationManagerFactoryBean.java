 ...
 *
 * <p>
 * This factory offers properties that in turn result in configuring the underlying
 * strategies. The table below shows the property names, their default settings, as well
 * as the strategies that they help to configure:
 *
 * <table>
 * <tr>
 ...
 *
 * <p>
 * Alternatively you can avoid use of the above convenience builder methods and set the
 * exact strategies to use via {@link #setStrategies(List)}.
 *
 * <p>
 * <strong>Deprecation Note:</strong> As of 5.2.4, {@link #setFavorPathExtension(boolean)
 * favorPathExtension} and {@link #setIgnoreUnknownPathExtensions(boolean)
 * ignoreUnknownPathExtensions} are deprecated in order to discourage using path
 * extensions for content negotiation and for request mapping with similar deprecations on
...
public class ContentNegotiationManagerFactoryBean
	 ...
	 * <p>
	 * <strong>Note:</strong> use of this method is mutually exclusive with use of all
	 * other setters in this class which customize a default, fixed set of strategies. See
	 * class level doc for more details.
	 ...
	 * <p>
	 * By default this is set to {@code true} in which case a request for
	 * {@code /hotels.pdf} will be interpreted as a request for {@code "application/pdf"}
	 * regardless of the 'Accept' header.
	 * @deprecated as of 5.2.4. See class-level note on the deprecation of path extension
	 * config options. As there is no replacement for this method, for the time being it's
	 * necessary to continue using it in order to set it to {@code false}. In 5.3 when
	 * {@code false} becomes the default, use of this property will no longer be
	 * necessary.
	 */
	@Deprecated
	...
	/**
	 * Add a mapping from a key to a MediaType where the key are normalized to lowercase
	 * and may have been extracted from a path extension, a filename extension, or passed
	 * as a query parameter.
	 * <p>
	 * The {@link #setFavorParameter(boolean) parameter strategy} requires such mappings
	 * in order to work while the {@link #setFavorPathExtension(boolean) path extension
	 * strategy} can fall back on lookups via {@link ServletContext#getMimeType} and
	 * {@link org.springframework.http.MediaTypeFactory}.
	 * <p>
	 * <strong>Note:</strong> Mappings registered here may be accessed via
	 * {@link ContentNegotiationManager#getMediaTypeMappings()} and may be used not only
	 * in the parameter and path extension strategies. For example, with the Spring MVC
	 * config, e.g. {@code @EnableWebMvc} or {@code <mvc:annotation-driven>}, the media
	 * type mappings are also plugged in to:
	 * <ul>
	 ...
	 * <li>List safe extensions for RFD attack detection (check the Spring Framework
	...
	public void setMediaTypes(Properties mediaTypes) {
		if (!CollectionUtils.isEmpty(mediaTypes)) {
			mediaTypes.forEach((key, value) -> addMediaType((String) key, MediaType.valueOf((String) value)));
		}
	}
	...
	/**
	 * Whether to ignore requests with path extension that cannot be resolved to any media
	 * type. Setting this to {@code false} will result in an
	 ...
	 * {@link #setUseRegisteredExtensionsOnly(boolean)}, which has reverse behavior.
	 */
	@Deprecated
	 ...
	 * {@link #setFavorParameter(boolean)} is set, this property determines whether to use
	 * only registered {@code MediaType} mappings or to allow dynamic resolution, e.g. via
	 * {@link MediaTypeFactory}.
	 * <p>
	...
	/**
	 * Whether a request parameter ("format" by default) should be used to determine the
	 * requested media type. For this option to work you must register
	 * {@link #setMediaTypes media type mappings}.
	 * <p>
	...
	/**
	 * Set a custom {@link ContentNegotiationStrategy} to use to determine the content
	 * type to use when no content type is requested.
	 * <p>
	 * By default this is not set.
	 * @since 4.1.2
	...
	public ContentNegotiationManager build() {
		...
		if (this.strategies != null) {
			...
			if (this.favorParameter) {
				...
				if (this.useRegisteredExtensionsOnly != null) {
					...
					strategy.setUseRegisteredExtensionsOnly(true); // backwards
																	// compatibility
				}
				strategies.add(strategy);
			}
		...
		// Ensure media type mappings are available via
		...
		if (!CollectionUtils.isEmpty(this.mediaTypes) && !this.favorPathExtension && !this.favorParameter) {
			this.contentNegotiationManager
					.addFileExtensionResolvers(new MappingMediaTypeFileExtensionResolver(this.mediaTypes));
		}
