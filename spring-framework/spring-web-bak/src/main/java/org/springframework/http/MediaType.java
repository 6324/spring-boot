...
/**
 * A subclass of {@link MimeType} that adds support for quality parameters as defined in
 ...
 * @author Sam Brannen
 * @since 3.0
 * @see <a href="https://tools.ietf.org/html/rfc7231#section-3.1.1.1"> HTTP 1.1: Semantics
...
public class MediaType extends MimeType implements Serializable {
	 ...
	 * @deprecated as of 5.2 in favor of {@link #APPLICATION_JSON} since major browsers
	 * like Chrome <a href="https://bugs.chromium.org/p/chromium/issues/detail?id=438464">
	 ...
	 * <a href="https://bugs.chromium.org/p/chromium/issues/detail?id=438464"> now comply
	 * with the specification</a> and interpret correctly UTF-8 special characters without
	 * requiring a {@code charset=UTF-8} parameter.
	 */
	@Deprecated
	 ...
	 * @see <a href="https://tools.ietf.org/html/rfc7807#section-6.1"> Problem Details for
	 * HTTP APIs, 6.1. application/problem+json</a>
	 * @deprecated as of 5.2 in favor of {@link #APPLICATION_PROBLEM_JSON} since major
	 * browsers like Chrome
	 * <a href="https://bugs.chromium.org/p/chromium/issues/detail?id=438464"> now comply
	 * with the specification</a> and interpret correctly UTF-8 special characters without
	 * requiring a {@code charset=UTF-8} parameter.
	 ...
	 * @since 5.0
	 * @deprecated as of 5.2 in favor of {@link #APPLICATION_PROBLEM_JSON_VALUE} since
	 * major browsers like Chrome
	 * <a href="https://bugs.chromium.org/p/chromium/issues/detail?id=438464"> now comply
	 * with the specification</a> and interpret correctly UTF-8 special characters without
	 * requiring a {@code charset=UTF-8} parameter.
	 */
	...
	/**
	 * Return the quality factor, as indicated by a {@code q} parameter, if any. Defaults
	 * to {@code 1.0}.
	 ...
	 * {@code application/*+xml} includes {@code application/soap+xml}, etc. This method
	 * is <b>not</b> symmetric.
	 * <p>
	 ...
	 * {@code MediaType}.
	 * @return the same instance if the given MediaType doesn't have a quality value, or a
	 * new one otherwise
	 */
	 ...
	 * @return the same instance if the media type doesn't contain a quality value, or a
	 * new one otherwise
	 */
	...
	/**
	 * Parse the given String value into a {@code MediaType} object, with this method name
	 * following the 'valueOf' naming convention (as supported by
	 ...
	 * <ol>
	 * <li>if either media type has a {@linkplain #isWildcardType() wildcard type}, then
	 * the media type without the wildcard is ordered before the other.</li>
	 * <li>if the two media types have different {@linkplain #getType() types}, then they
	 * are considered equal and remain their current order.</li>
	 * <li>if either media type has a {@linkplain #isWildcardSubtype() wildcard subtype},
	 * then the media type without the wildcard is sorted before the other.</li>
	 * <li>if the two media types have different {@linkplain #getSubtype() subtypes}, then
	 * they are considered equal and remain their current order.</li>
	 * <li>if the two media types have different {@linkplain #getQualityValue() quality
	 * value}, then the media type with the highest quality value is ordered before the
	 * other.</li>
	 * <li>if the two media types have a different amount of
	 * {@linkplain #getParameter(String) parameters}, then the media type with the most
	...
	/**
	 * Sorts the given list of {@code MediaType} objects by specificity as the primary
	 * criteria and quality value the secondary.
