...
public class HttpHeadResponseDecorator extends ServerHttpResponseDecorator {
	 ...
	 * <p>
	 * If the headers contain neither Content-Length nor Transfer-Encoding, count the
	 * bytes and set Content-Length.
	 */
	@Override
	public final Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
		if (shouldSetContentLength()) {
			return Flux.from(body).reduce(0, (current, buffer) -> {
				...
				return next;
			}).doOnNext(length -> getHeaders().setContentLength(length)).then();
		}
		else {
	 ...
	 * that can be computed on a streaming response.
	 */
	@Override
