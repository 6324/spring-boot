...
class ProtobufIntegrationTests extends AbstractRequestMappingIntegrationTests {
	...
	static final Msg TEST_MSG = Msg.newBuilder().setFoo("Foo").setBlah(SecondMsg.newBuilder().setBlah(123).build())
			.build();
	...
	static class ProtobufController {
		...
		Flux<Msg> messageStream() {
			return testInterval(Duration.ofMillis(50), 5).map(l -> Msg.newBuilder().setFoo("Foo")
					.setBlah(SecondMsg.newBuilder().setBlah(l.intValue()).build()).build());
		}
