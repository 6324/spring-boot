...
abstract class WebClientUtils {
	...
	public static <T> Mono<ResponseEntity<T>> mapToEntity(ClientResponse response, Mono<T> bodyMono) {
		return ((Mono<Object>) bodyMono).defaultIfEmpty(VALUE_NONE)
				.map(body -> ResponseEntity.status(response.rawStatusCode()).headers(response.headers().asHttpHeaders())
	...
	public static <T> Mono<ResponseEntity<List<T>>> mapToEntityList(ClientResponse response, Publisher<T> body) {
		return Flux.from(body).collectList().map(list -> ResponseEntity.status(response.rawStatusCode())
				.headers(response.headers().asHttpHeaders()).body(list));
	}
