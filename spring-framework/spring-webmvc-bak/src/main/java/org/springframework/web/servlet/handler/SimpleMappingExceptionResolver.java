...
/**
 * {@link org.springframework.web.servlet.HandlerExceptionResolver} implementation that
 * allows for mapping exception class names to view names, either for a set of given
 * handlers or for all handlers in the DispatcherServlet.
 *
 * <p>
...
public class SimpleMappingExceptionResolver extends AbstractHandlerExceptionResolver {
	...
	/**
	 * Set the mappings between exception class names and error view names. The exception
	 * class name can be a substring, with no wildcard support at present. A value of
	 ...
	 * package information (which isn't mandatory). For example, "Exception" will match
	 ...
	 * keys, and error view names as values
	 */
	...
	/**
	 * Set one or more exceptions to be excluded from the exception mappings. Excluded
	 * exceptions are checked first and if one of them equals the actual exception, the
	 * exception will remain unresolved.
	...
	/**
	 * Set the name of the default error view. This view will be returned if no specific
	 * mapping was found.
	 * <p>
	 * Default is none.
	 */
	 ...
	 * <p>
	 * Note that this error code will only get applied in case of a top-level request. It
	 * will not be set for an include request, since the HTTP status cannot be modified
	...
	/**
	 * An alternative to {@link #setStatusCodes(Properties)} for use with Java-based
	 * configuration.
	 */
	...
	/**
	 * Set the default HTTP status code that this exception resolver will apply if it
	 * resolves an error view and if there is no status code mapping defined.
	 * <p>
	 * Note that this error code will only get applied in case of a top-level request. It
	 * will not be set for an include request, since the HTTP status cannot be modified
	 * from within an include.
	 * <p>
	 * If not specified, no status code will be applied, either leaving this to the
	 ...
	 * <p>
	 * This can be either set to a different attribute name or to {@code null} for not
	 * exposing an exception attribute at all.
	 ...
	 * @return a corresponding {@code ModelAndView} to forward to, or {@code null} for
	 * default processing in the resolution chain
	 */
	@Override
	@Nullable
	 ...
	 * @param exceptionMappings mappings between exception class names and error view
	 * names
	 * @param ex the exception that got thrown during handler execution
	 ...
	 * <p>
	 * 0 means ex matches exactly. Returns -1 if there's no match. Otherwise, returns
	 ...
	 * <p>
	 * The default implementation returns the status code for the given view name
	 * (specified through the {@link #setStatusCodes(Properties) statusCodes} property),
	 * or falls back to the {@link #setDefaultStatusCode defaultStatusCode} if there is no
	 * match.
	 * <p>
	 * Override this in a custom subclass to customize this behavior.
	 * @param request current HTTP request
	 * @param viewName the name of the error view
	 * @return the HTTP status code to use, or {@code null} for the servlet container's
	 * default (200 in case of a standard error view)
