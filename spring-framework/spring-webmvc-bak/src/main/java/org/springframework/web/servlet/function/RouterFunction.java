...
public interface RouterFunction<T extends ServerResponse> {
	...
	/**
	 * Return the {@linkplain HandlerFunction handler function} that matches the given
	 * request.
	 * @param request the request to route
	 * @return an {@code Optional} describing the {@code HandlerFunction} that matches
	 * this request, or an empty {@code Optional} if there is no match
	 */
	Optional<HandlerFunction<T>> route(ServerRequest request);

	/**
	 * Return a composed routing function that first invokes this function, and then
	 * invokes the {@code other} function (of the same response type {@code T}) if this
	 * route had {@linkplain Optional#empty() no result}.
	 * @param other the function of type {@code T} to apply when this function has no
	 * result
	...
	/**
	 * Return a composed routing function that first invokes this function, and then
	 * invokes the {@code other} function (of a different response type) if this route had
	...
	/**
	 * Return a composed routing function that routes to the given handler function if
	 * this route does not match and the given request predicate applies. This method is a
	 * convenient combination of {@link #and(RouterFunction)} and
	 ...
	 * @param handlerFunction the handler function to route to if this route does not
	 * match and the predicate applies
	 * @return a composed function that route to {@code handlerFunction} if this route
	 * does not match and if {@code predicate} applies
	 */
	 ...
	 * route does not match and the given request predicate applies. This method is a
	 * convenient combination of {@link #and(RouterFunction)} and
	 * {@link RouterFunctions#nest(RequestPredicate, RouterFunction)}.
	 * @param predicate the predicate to test if this route does not match
	 * @param routerFunction the router function to route to if this route does not match
	 * and the predicate applies
	 * @return a composed function that route to {@code routerFunction} if this route does
	 * not match and if {@code predicate} applies
	 */
	 ...
	 * {@link RouterFunctions.Visitor#unknown(RouterFunction)}; composed
	 * {@code RouterFunction} implementations are expected to call {@code accept} for all
	 * components that make up this router function.
