...
/**
 * Common base class for all containers which need to implement listening based on a JMS
 * Connection (either shared or freshly obtained for each attempt). Inherits basic
 * Connection and Session configuration handling from the
 * {@link org.springframework.jms.support.JmsAccessor} base class.
 *
 * <p>
 * This class provides basic lifecycle management, in particular management of a shared
 * JMS Connection. Subclasses are supposed to plug into this lifecycle, implementing the
 * {@link #sharedConnectionEnabled()} as well as the {@link #doInitialize()} and
 * {@link #doShutdown()} template methods.
 *
 * <p>
 * This base class does not assume any specific listener programming model or listener
 * invoker mechanism. It just provides the general runtime lifecycle management needed for
 * any kind of JMS-based listening mechanism that operates on a JMS Connection/Session.
 *
 * <p>
 * For a concrete listener programming model, check out the
 * {@link AbstractMessageListenerContainer} subclass. For a concrete listener invoker
 * mechanism, check out the {@link DefaultMessageListenerContainer} class.
 *
...
public abstract class AbstractJmsListeningContainer extends JmsDestinationAccessor
	...
	/**
	 * Specify the JMS client ID for a shared Connection created and used by this
	 * container.
	 * <p>
	 * Note that client IDs need to be unique among all active Connections of the
	 * underlying JMS provider. Furthermore, a client ID can only be assigned if the
	 * original ConnectionFactory hasn't already assigned one.
	...
	/**
	 * Return the JMS client ID for the shared Connection created and used by this
	 * container, if any.
	 */
	@Nullable
	 ...
	 * <p>
	 * Default is "true"; set this to "false" to allow for manual startup through the
	 * {@link #start()} method.
	 */
	...
	/**
	 * Specify the phase in which this container should be started and stopped. The
	 ...
	 * container starts as late as possible and stops as soon as possible.
	 */
	...
	/**
	 * Return the bean name that this listener container has been assigned in its
	 * containing bean factory, if any.
	 */
	@Nullable
	 ...
	 * <p>
	 * Creates a JMS Connection, starts the {@link javax.jms.Connection} (if
	 * {@link #setAutoStartup(boolean) "autoStartup"} hasn't been turned off), and calls
	 * {@link #doInitialize()}.
	...
	/**
	 * Return whether this container is currently active, that is, whether it has been set
	 * up but not shut down yet.
	 */
	...
	/**
	 * Determine whether this container is currently running, that is, whether it has been
	 ...
	 * <p>
	 * This implementation always returns {@code true}; the default 'running' state is
	 * purely determined by {@link #start()} / {@link #stop()}.
	 * <p>
	 * Subclasses may override this method to check against temporary conditions that
	 * prevent listeners from actually running. In other words, they may apply further
	 * restrictions to the 'running' state, returning {@code false} if such a restriction
	 * prevents listeners from running.
	 */
	 ...
	 * <p>
	 * The default implementation delegates to {@link #createSharedConnection()}, which
	 * does one immediate attempt and throws an exception if it fails. Can be overridden
	 * to have a recovery process in place, retrying until a Connection can be
	 ...
	 * <p>
	 * The default implementation creates a standard Connection and prepares it through
	 * {@link #prepareSharedConnection}.
	...
	/**
	 * Prepare the given Connection, which is about to be registered as shared Connection
	 * for this container.
	 * <p>
	 * The default implementation sets the specified client id, if any. Subclasses can
	 * override this to apply further settings.
	...
	/**
	 * Return the shared JMS Connection maintained by this container. Available after
	 * initialization.
	 * @return the shared Connection (never {@code null})
	 * @throws IllegalStateException if this container does not maintain a shared
	 * Connection, or if the Connection hasn't been initialized yet
	 * @see #sharedConnectionEnabled()
	 */
	...
	/**
	 * Take the given task object and reschedule it, either immediately if this container
	 * is currently running, or later once this container has been restarted.
	 * <p>
	 * If this container has already been shut down, the task will not get rescheduled at
	 * all.
	 * @param task the task object to reschedule
	 * @return whether the task has been rescheduled (either immediately or for a restart
	 * of this container)
	...
	/**
	 * Try to resume all paused tasks. Tasks for which rescheduling failed simply remain
	 * in paused mode.
	 */
	 ...
	 * {@code rescheduleTaskIfNecessary}. This implementation throws an
	...
	/**
	 * Return whether a shared JMS Connection should be maintained by this container base
	 * class.
	 ...
	 * <p>
	 * Subclasses need to implement this method for their specific invoker management
	 * process.
	 * <p>
	 ...
	 * process.
	 * <p>
	 * A shared JMS Connection, if any, will automatically be closed <i>afterwards</i>.
	 * @throws JMSException if shutdown failed
	 * @see #shutdown()
	 */
	...
	/**
	 * Exception that indicates that the initial setup of this container's shared JMS
	 * Connection failed. This is indicating to invokers that they need to establish the
	 * shared Connection themselves on first access.
	 */
