...
/**
 * Base class for {@link org.springframework.jms.core.JmsTemplate} and other JMS-accessing
 * gateway helpers, defining common properties such as the JMS {@link ConnectionFactory}
 ...
 *
 * <p>
 * Not intended to be used directly. See {@link org.springframework.jms.core.JmsTemplate}.
 *
...
public abstract class JmsAccessor implements InitializingBean {
	...
	/**
	 * Set the transaction mode that is used when creating a JMS {@link Session}. Default
	 * is "false".
	 * <p>
	 ...
	 * are not taken into account. Depending on the Java EE transaction context, the
	 * container makes its own decisions on these values. Analogously, these parameters
	 * are not taken into account within a locally managed transaction either, since the
	 * accessor operates on an existing JMS Session in this case.
	 * <p>
	 * Setting this flag to "true" will use a short local JMS transaction when running
	 * outside of a managed transaction, and a synchronized local JMS transaction in case
	 * of a managed transaction (other than an XA transaction) being present. This has the
	 * effect of a local JMS transaction being managed alongside the main transaction
	 * (which might be a native JDBC transaction), with the JMS transaction committing
	...
	/**
	 * Return whether the JMS {@link Session sessions} used by this accessor are supposed
	 * to be transacted.
	 ...
	 * <p>
	 * If you want to use vendor-specific extensions to the acknowledgment mode, use
	 * {@link #setSessionAcknowledgeMode(int)} instead.
	...
	/**
	 * Set the JMS acknowledgement mode that is used when creating a JMS {@link Session}
	 * to send a message.
	 * <p>
	 ...
	 * {@code create(Queue/Topic)Session(boolean transacted, int acknowledgeMode)} method
	 * are not taken into account. Depending on the transaction context in the EJB, the
	 * container makes its own decisions on these values. See section 17.3.5 of the EJB
	 * spec.
