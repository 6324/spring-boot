 ...
 * <p>
 * Given two arrays of equal length and varying types, the standard technique for sorting
 * them in parallel is to create a new temporary object for each row, store the objects in
 * a temporary array, sort the array using a custom comparator, and the extract the
 * original values back into their respective arrays. This is wasteful in both time and
 * memory.
 * <p>
 * This class generates bytecode customized to the particular set of arrays you need to
 * sort, in such a way that both arrays are sorted in-place, simultaneously.
 * <p>
 * Two sorting algorithms are provided. Quicksort is best when you only need to sort by a
 * single column, as it requires very few comparisons and swaps. Mergesort is best used
 * when sorting multiple columns, as it is a "stable" sort--that is, it does not affect
 * the relative order of equal objects from previous sorts.
 * <p>
 * The mergesort algorithm here is an "in-place" variant, which while slower, does not
...
abstract public class ParallelSorter extends SorterTemplate {
	...
	/**
	 * Create a new ParallelSorter object for a set of arrays. You may sort the arrays
	 * multiple times via the same ParallelSorter object.
	 * @param arrays An array of arrays to sort. The arrays may be a mix of primitive and
	 * non-primitive types, but should all be the same length.
