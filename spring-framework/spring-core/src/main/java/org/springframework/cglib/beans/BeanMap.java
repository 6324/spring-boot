...
/**
 * A <code>Map</code>-based view of a JavaBean. The default set of keys is the union of
 * all property names (getters or setters). An attempt to set a read-only property will be
 * ignored, and write-only properties will be returned as <code>null</code>. Removal of
 * objects is not a supported (the key set is fixed).
 *
