...
/**
 * Helper class for ProxyObjectInstantiator. We can see the details of a class
 * specification
...
public final class ClassDefinitionUtils {
	...
	public static final int ACC_PUBLIC = 0x0001; // Declared public; may be accessed from
													// outside its package.

	public static final int ACC_FINAL = 0x0010; // Declared final; no subclasses allowed.

	public static final int ACC_SUPER = 0x0020; // Treat superclass methods specially when
												// invoked by the invokespecial
												// instruction.

	public static final int ACC_INTERFACE = 0x0200; // Is an interface, not a class.

	public static final int ACC_ABSTRACT = 0x0400; // Declared abstract; must not be
													// instantiated.

	public static final int ACC_SYNTHETIC = 0x1000; // Declared synthetic; not present in
													// the source code.
	...
	public static final byte[] VERSION = { (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x31 }; // minor_version,
																									// major_version
																									// (Java
																									// 5)
	...
	static {
		PROTECTION_DOMAIN = AccessController
				.doPrivileged((PrivilegedAction<ProtectionDomain>) ClassDefinitionUtils.class::getProtectionDomain);
	}

	/**
	 * Define a class in the provided class loader from the array of bytes. Inspired by
	 * cglib <code>ReflectUtils.defineClass</code>
	 * @param <T> type of the class returned
	 * @param className class name in the format
	 * <code>org.springframework.objenesis..MyClass</code>
	...
	public static <T> Class<T> defineClass(String className, byte[] b, Class<?> neighbor, ClassLoader loader)
			throws Exception {
		Class<T> c = (Class<T>) DefineClassHelper.defineClass(className, b, 0, b.length, neighbor, loader,
				PROTECTION_DOMAIN);
	...
	public static byte[] readClass(String className) throws IOException {
		...
		byte[] b = new byte[2500]; // I'm assuming that I'm reading class that are not too
									// big
