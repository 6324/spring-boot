...
public final class PlatformDescription {
	...
	public static String describePlatform() {
		String desc = "Java " + SPECIFICATION_VERSION + " (" + "VM vendor name=\"" + VENDOR + "\", "
				+ "VM vendor version=" + VENDOR_VERSION + ", " + "JVM name=\"" + JVM_NAME + "\", " + "JVM version="
				+ VM_VERSION + ", " + "JVM info=" + VM_INFO;
	...
	/**
	 * Check if the current JVM is of the type passed in parameter. Normally, this will be
	 * a constant from this class. We basically do
	 ...
	 * @return if it's an Android version based on the OpenJDK. Will return false if this
	 * JVM isn't an Android JVM at all
	 */
	...
	private static boolean getIsAndroidOpenJDK() {
		...
		// Sadly, Android N is still API 23. So we can't base ourselves on the API level
		// to know if it is an OpenJDK
		// version or not
	...
	public static boolean isAfterJigsaw() {
		...
		return version.indexOf('.') < 0; // No dot means the version is 9, 10, 11, ... not
											// 1.6, 1.7, 1.8
	}
