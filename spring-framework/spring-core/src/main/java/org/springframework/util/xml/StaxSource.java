...
/**
 * Implementation of the {@code Source} tagging interface for StAX readers. Can be
 * constructed with an {@code XMLEventReader} or an {@code XMLStreamReader}.
 *
 * <p>
 * This class is necessary because there is no implementation of {@code Source} for StAX
 * Readers in JAXP 1.3. There is a {@code StAXSource} in JAXP 1.4 (JDK 1.6), but this
 * class is kept around for backwards compatibility reasons.
 *
 * <p>
 * Even though {@code StaxSource} extends from {@code SAXSource}, calling the methods of
 * {@code SAXSource} is <strong>not supported</strong>. In general, the only supported
 * operation on this class is to use the {@code XMLReader} obtained via
 ...
 * {@code UnsupportedOperationException #setInputSource(InputSource)} will result in
 * {@code UnsupportedOperationExceptions}.
 *
...
class StaxSource extends SAXSource {
	...
	/**
	 * Construct a new instance of the {@code StaxSource} with the specified
	 * {@code XMLEventReader}. The supplied event reader must be in
	...
	/**
	 * Construct a new instance of the {@code StaxSource} with the specified
	 * {@code XMLStreamReader}. The supplied stream reader must be in
	 * {@code XMLStreamConstants.START_DOCUMENT} or
	 * {@code XMLStreamConstants.START_ELEMENT} state.
	 ...
	 * <p>
	 * If this {@code StaxSource} was created with an {@code XMLStreamReader}, the result
	 * will be {@code null}.
	 ...
	 * <p>
	 * If this {@code StaxSource} was created with an {@code XMLEventReader}, the result
	 * will be {@code null}.
	 * @return the StAX event reader used by this source
	 * @see StaxSource#StaxSource(javax.xml.stream.XMLEventReader)
	 */
