 ...
 * {@link Method} or {@link Constructor} plus a parameter index and a nested type index
 * for a declared generic type. Useful as a specification object to pass along.
 *
 * <p>
 * As of 4.2, there is a
 * {@link org.springframework.core.annotation.SynthesizingMethodParameter} subclass
 * available which synthesizes annotations with attribute aliases. That subclass is used
...
public class MethodParameter {
	...
	/**
	 * Internal constructor used to create a {@link MethodParameter} with a containing
	 * class already set.
	...
	/**
	 * Copy constructor, resulting in an independent MethodParameter object based on the
	 ...
	 * <p>
	 * Note: This method exposes the annotations declared on the method/constructor itself
	 ...
	 * @see #getNestingLevel()
	 * @deprecated since 5.2 in favor of retaining the original MethodParameter and using
	 * {@link #nested(Integer)} if nesting is required
	 */
	@Deprecated
	...
	/**
	 * Return a variant of this {@code MethodParameter} which points to the same parameter
	 * but one nesting level deeper.
	 * @since 4.3
	 */
	...
	/**
	 * Return whether this method indicates a parameter which is not required: either in
	 * the form of Java 8's {@link java.util.Optional}, any variant of a parameter-level
	 * {@code Nullable} annotation (such as from JSR-305 or the FindBugs set of
	 * annotations), or a language-level nullable type declaration or {@code Continuation}
	 * parameter in Kotlin.
	 * @since 4.3
	 */
	...
	/**
	 * Return a variant of this {@code MethodParameter} which points to the same parameter
	 * but one nesting level deeper in case of a {@link java.util.Optional} declaration.
	 * @since 4.3
	 * @see #isOptional()
	 * @see #nested()
	 */
		...
		if (paramType == null) {
			if (this.parameterIndex < 0) {
				...
				paramType = (method != null
						? (KotlinDetector.isKotlinReflectPresent() && KotlinDetector.isKotlinType(getContainingClass())
								? KotlinDelegate.getGenericReturnType(method) : method.getGenericReturnType())
						: void.class);
			}
			else {
	...
	public Annotation[] getParameterAnnotations() {
		...
		if (paramAnns == null) {
			...
			if (this.executable instanceof Constructor && ClassUtils.isInnerClass(this.executable.getDeclaringClass())
				...
				// parameter
				// for inner classes, so access it with the actual parameter index lowered
				// by 1
				index = this.parameterIndex - 1;
			}
	...
	/**
	 * Return {@code true} if the parameter has at least one annotation, {@code false} if
	 * it has none.
	 ...
	 * @return the parameter name (may be {@code null} if no parameter name metadata is
	 * contained in the class file or no {@link #initParameterNameDiscovery
	 * ParameterNameDiscoverer} has been set to begin with)
	 */
	@Nullable
	...
	/**
	 * A template method to post-process a given annotation instance before returning it
	 * to the caller.
	 * <p>
	...
	/**
	 * A template method to post-process a given annotation array before returning it to
	 * the caller.
	 * <p>
	...
	public String toString() {
		Method method = getMethod();
		return (method != null ? "method '" + method.getName() + "'" : "constructor") + " parameter "
				+ this.parameterIndex;
	}
	...
	private static class KotlinDelegate {
		...
		/**
		 * Check whether the specified {@link MethodParameter} represents a nullable
		 * Kotlin type, an optional parameter (with a default value in the Kotlin
		 * declaration) or a {@code Continuation} parameter used in suspending functions.
		 */
		...
		/**
		 * Return the return type of the method, with support of suspending functions via
		 * Kotlin reflection.
		 */
