...
/**
 * Strategy to encode a stream of Objects of type {@code <T>} into an output stream of
 * bytes.
 *
...
public interface Encoder<T> {
	...
	/**
	 * Whether the encoder supports the given source element type and the MIME type for
	 * the output stream.
	 * @param elementType the type of elements in the source stream
	 * @param mimeType the MIME type for the output stream (can be {@code null} if not
	 * specified)
	...
	/**
	 * Encode a stream of Objects of type {@code T} into a {@link DataBuffer} output
	 * stream.
	 ...
	 * @param elementType the expected type of elements in the input stream; this type
	 * must have been previously passed to the {@link #canEncode} method and it must have
	 * returned {@code true}.
	 ...
	 * distinct messages (or events) are encoded and handled individually, in fully
	 * aggregated form.
	 * <p>
	 * By default this method raises {@link UnsupportedOperationException} and it is
	 * expected that some encoders cannot produce a single buffer or cannot do so
	 * synchronously (e.g. encoding a {@code Resource}).
