 ...
 * traditional {@code main} method accepting a {@code String[]} of arguments from the
 ...
 * inject arguments as values into Spring beans. It is this latter set of cases in which a
 * {@code CommandLinePropertySource} becomes useful. A {@code CommandLinePropertySource}
 ...
 * chosen from the command line property source first. This is a reasonable approach given
 * that arguments specified on the command line are naturally more specific than those
 * specified as environment variables.
 *
 * <p>
 * As an alternative to injecting the {@code Environment}, Spring's {@code @Value}
 * annotation may be used to inject these properties, given that a
 * {@link PropertySourcesPropertyResolver} bean has been registered, either directly or
 * through using the {@code <context:property-placeholder>} element. For example:
 *
 * <pre class="code">
 * &#064;Component
 ...
 *
 * <p>
 * Note also that while "--" was used in the examples above to denote an option argument,
 * this syntax may vary across individual command line argument libraries. For example, a
 * JOpt- or Commons CLI-based implementation may allow for single dash ("-") "short"
 ...
 * arguments" and available through the special
 * {@linkplain #DEFAULT_NON_OPTION_ARGS_PROPERTY_NAME "nonOptionArgs"} property. If
 * multiple non-option arguments are specified, the value of this property will be a
 ...
 * CommandLinePropertySource} and at the same time lends itself to conversion when used in
 * conjunction with the Spring {@link Environment} and its built-in {@code
 ...
 * {@link #setNonOptionArgsPropertyName(String)} method. Doing so is recommended as it
 * gives proper semantic value to non-option arguments. For example, if filesystem paths
 * are being specified as non-option arguments, it is likely preferable to refer to these
 * as something like "file.locations" than the default of "nonOptionArgs":
 *
 * <pre class="code">
 * public static void main(String[] args) {
 *     CommandLinePropertySource clps = ...;
 ...
 * <em>after</em> they have been parsed. So the typical case will involve fully
 * configuring the underlying command line parsing API, parsing the {@code String[]} of
 * arguments coming into the main method, and then simply providing the parsing results to
 * an implementation of {@code CommandLinePropertySource}. At that point, all arguments
 * can be considered either 'option' or 'non-option' arguments and as described above can
...
public abstract class CommandLinePropertySource<T> extends EnumerablePropertySource<T> {
	 ...
	 * and if so delegates to the abstract {@link #getNonOptionArgs()} method checking to
	 * see whether it returns an empty collection. Otherwise delegates to and returns the
	 * value of the abstract {@link #containsOption(String)} method.
	 */
	@Override
	 ...
	 * This implementation first checks to see if the name specified is the special
	 * {@linkplain #setNonOptionArgsPropertyName(String) "non-option arguments" property},
	 * and if so delegates to the abstract {@link #getNonOptionArgs()} method. If so and
	 * the collection of non-option arguments is empty, this method returns {@code
	 * null}. If not empty, it returns a comma-separated String of all non-option
	 * arguments. Otherwise delegates to and returns the result of the abstract
	 * {@link #getOptionValues(String)} method.
	 */
	@Override
	@Nullable
	...
	/**
	 * Return whether the set of option arguments parsed from the command line contains an
