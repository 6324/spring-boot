...
/**
 * Contextual descriptor about a type to convert from or to. Capable of representing
 * arrays and generic collection types.
 *
 * @author Keith Donald
 * @author Andy Clement
...
public class TypeDescriptor implements Serializable {
	 ...
	 * <p>
	 * Use this constructor when a source or target conversion point is a constructor
	 * parameter, method parameter, or method return value.
	 ...
	 * <p>
	 * Use this constructor when a source or target conversion point is a property on a
	 * Java class.
	 ...
	 * <p>
	 * This constructor is used internally and may also be used by subclasses that support
	 * non-Java languages with extended type systems. It is public as of 5.1.4 whereas it
	...
	/**
	 * Variation of {@link #getType()} that accounts for a primitive type by returning its
	 * object wrapper type.
	 * <p>
	 * This is useful for conversion service implementations that wish to normalize to
	 * object-based types and not work with primitive types directly.
	 */
	...
	/**
	 * The type of the backing class, method parameter, field, or property described by
	 * this TypeDescriptor.
	 * <p>
	 * Returns primitive types as-is. See {@link #getObjectType()} for a variation of this
	 * operation that resolves primitive types to their corresponding Object types if
	 * necessary.
	 * @see #getObjectType()
	 */
	 ...
	 * provided value.
	 * <p>
	 * If the value is {@code null}, no narrowing is performed and this TypeDescriptor is
	 * returned unchanged.
	 * <p>
	 * Designed to be called by binding frameworks when they read property, field, or
	 * method return values. Allows such frameworks to narrow a TypeDescriptor built from
	 * a declared property, field, or method return value type. For example, a field
	 * declared as {@code java.lang.Object} would be narrowed to {@code java.util.HashMap}
	 * if it was set to a {@code java.util.HashMap} value. The narrowed TypeDescriptor can
	 * then be used to convert the HashMap to some other type. Annotation and nested type
	 * context is preserved by the narrowed copy.
	...
	/**
	 * Obtain the annotation of the specified {@code annotationType} that is on this type
	 * descriptor.
	 * <p>
	 * As of Spring Framework 4.2, this method supports arbitrary levels of
	 * meta-annotations.
	 * @param annotationType the annotation type
	 * @return the annotation, or {@code null} if no such annotation exists on this type
	 * descriptor
	 */
	@Nullable
	 ...
	 * <p>
	 * For example,
	 * {@code valueOf(String.class).isAssignableTo(valueOf(CharSequence.class))} returns
	 * {@code true} because a String value can be assigned to a CharSequence variable. On
	 * the other hand,
	 ...
	 * Collection&lt;CharSequence&gt; field, but List&lt;Number&gt; is not assignable to
	 * List&lt;Integer&gt;.
	 * @return {@code true} if this type is assignable to the type represented by the
	...
	/**
	 * If this type is an array, returns the array's component type. If this type is a
	 * {@code Stream}, returns the stream's component type. If this type is a
	 * {@link Collection} and it is parameterized, returns the Collection's element type.
	 * If the Collection is not parameterized, returns {@code null} indicating the element
	 * type is not declared.
	 * @return the array component type or Collection element type, or {@code null} if
	 * this type is not an array type or a {@code java.util.Collection} or if its element
	 ...
	 * {@code java.lang.Integer}. If this describes a {@code java.util.List&lt;?&gt;} and
	 * the element argument is an {@code java.lang.Integer}, the returned TypeDescriptor
	 * will be {@code java.lang.Integer} as well.
	 * <p>
	 * Annotation and nested type context will be preserved in the narrowed TypeDescriptor
	 * that is returned.
	...
	/**
	 * If this type is a {@link Map} and its key type is parameterized, returns the map's
	 * key type. If the Map's key type is not parameterized, returns {@code null}
	 * indicating the key type is not declared.
	 * @return the Map key type, or {@code null} if this type is a Map but its key type is
	 * not parameterized
	...
	/**
	 * If this type is a {@link Map}, creates a mapKey {@link TypeDescriptor} from the
	 * provided map key.
	 * <p>
	 * Narrows the {@link #getMapKeyTypeDescriptor() mapKeyType} property to the class of
	 * the provided map key. For example, if this describes a
	 ...
	 * and the key argument is a {@code java.lang.Integer}, the returned TypeDescriptor
	 * will be {@code java.lang.Integer} as well.
	 * <p>
	 * Annotation and nested type context will be preserved in the narrowed TypeDescriptor
	 * that is returned.
	...
	/**
	 * If this type is a {@link Map} and its value type is parameterized, returns the
	 * map's value type.
	 * <p>
	 * If the Map's value type is not parameterized, returns {@code null} indicating the
	 * value type is not declared.
	 * @return the Map value type, or {@code null} if this type is a Map but its value
	 * type is not parameterized
	 * @throws IllegalStateException if this type is not a {@code java.util.Map}
	 */
	@Nullable
	...
	/**
	 * If this type is a {@link Map}, creates a mapValue {@link TypeDescriptor} from the
	 * provided map value.
	 * <p>
	 * Narrows the {@link #getMapValueTypeDescriptor() mapValueType} property to the class
	 * of the provided map value. For example, if this describes a
	 * {@code java.util.Map&lt;java.lang.String, java.lang.Number&lt;} and the value
	 * argument is a {@code java.lang.Integer}, the returned TypeDescriptor will be
	 * {@code java.lang.Integer}. If this describes a {@code java.util.Map&lt;?, ?&gt;}
	 * and the value argument is a {@code java.lang.Integer}, the returned TypeDescriptor
	 * will be {@code java.lang.Integer} as well.
	 * <p>
	 * Annotation and nested type context will be preserved in the narrowed TypeDescriptor
	 * that is returned.
	 ...
	 * <p>
	 * Use this factory method to introspect a source object before asking the conversion
	 * system to convert it to some another type.
	 * <p>
	 ...
	 * target type, when no type location such as a method parameter or field is available
	 * to provide additional conversion context.
	 * <p>
	 ...
	 * @param elementTypeDescriptor a descriptor for the collection's element type, used
	 * to convert collection elements
	 ...
	 * EmailAddress&gt; by converting to a targetType built with this method: The method
	 * call to construct such a TypeDescriptor would look something like:
	 * <pre class="code">
	 * map(Map.class, TypeDescriptor.valueOf(Id.class), TypeDescriptor.valueOf(EmailAddress.class));
	 * </pre>
	 * @param mapType the map type, which must implement {@link Map}
	 * @param keyTypeDescriptor a descriptor for the map's key type, used to convert map
	 * keys
	 ...
	 * <p>
	 * For example to create a {@code Map<String,String>[]} use: <pre class="code">
	 * TypeDescriptor.array(TypeDescriptor.map(Map.class, TypeDescriptor.value(String.class), TypeDescriptor.value(String.class)));
	 * </pre>
	 * @param elementTypeDescriptor the {@link TypeDescriptor} of the array element or
	 * {@code null}
	 ...
	 * <p>
	 * For example, if the methodParameter is a {@code List<String>} and the nesting level
	 * is 1, the nested type descriptor will be String.class.
	 * <p>
	 * If the methodParameter is a {@code List<List<String>>} and the nesting level is 2,
	 * the nested type descriptor will also be a String.class.
	 * <p>
	 * If the methodParameter is a {@code Map<Integer, String>} and the nesting level is
	 * 1, the nested type descriptor will be String, derived from the map value.
	 * <p>
	 * If the methodParameter is a {@code List<Map<Integer, String>>} and the nesting
	 * level is 2, the nested type descriptor will be String, derived from the map value.
	 * <p>
	 * Returns {@code null} if a nested type cannot be obtained because it was not
	 * declared. For example, if the method parameter is a {@code List<?>}, the nested
	 ...
	 * @return the nested type descriptor at the specified nesting level, or {@code null}
	 * if it could not be obtained
	 * @throws IllegalArgumentException if the nesting level of the input
	 * {@link MethodParameter} argument is not 1, or if the types up to the specified
	 ...
	 * <p>
	 * For example, if the field is a {@code List<String>} and the nesting level is 1, the
	 * nested type descriptor will be {@code String.class}.
	 * <p>
	 * If the field is a {@code List<List<String>>} and the nesting level is 2, the nested
	 * type descriptor will also be a {@code String.class}.
	 * <p>
	 * If the field is a {@code Map<Integer, String>} and the nesting level is 1, the
	 * nested type descriptor will be String, derived from the map value.
	 * <p>
	 * If the field is a {@code List<Map<Integer, String>>} and the nesting level is 2,
	 ...
	 * @return the nested type descriptor at the specified nesting level, or {@code null}
	 * if it could not be obtained
	 * @throws IllegalArgumentException if the types up to the specified nesting level are
	 * not of collection, array, or map types
	 */
	@Nullable
	 ...
	 * <p>
	 * For example, if the property is a {@code List<String>} and the nesting level is 1,
	 * the nested type descriptor will be {@code String.class}.
	 * <p>
	 * If the property is a {@code List<List<String>>} and the nesting level is 2, the
	 * nested type descriptor will also be a {@code String.class}.
	 * <p>
	 * If the property is a {@code Map<Integer, String>} and the nesting level is 1, the
	 * nested type descriptor will be String, derived from the map value.
	 * <p>
	 * If the property is a {@code List<Map<Integer, String>>} and the nesting level is 2,
	 * the nested type descriptor will be String, derived from the map value.
	 * <p>
	 * Returns {@code null} if a nested type cannot be obtained because it was not
