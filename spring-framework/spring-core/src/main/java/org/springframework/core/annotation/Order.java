 ...
 * components in Spring, even for collection injection where the order values of the
 * target components are taken into account (either from their target class or from their
 * {@code @Bean} method). While such order values may influence priorities at injection
 * points, please be aware that they do not influence singleton startup order which is an
 ...
 *
 * <p>
 * Since Spring 4.1, the standard {@link javax.annotation.Priority} annotation can be used
 * as a drop-in replacement for this annotation in ordering scenarios. Note that
 * {@code @Priority} may have additional semantics when a single element has to be picked
 ...
 * {@link Ordered} interface, allowing for configuration-determined instance values
 * instead of hard-coded values attached to a particular class.
 *
 * <p>
