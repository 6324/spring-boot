...
/**
 * A single merged annotation returned from a {@link MergedAnnotations} collection.
 * Presents a view onto an annotation where attribute values may have been "merged" from
 * different source values.
 *
 * <p>
 * Attribute values may be accessed using the various {@code get} methods. For example, to
 * access an {@code int} attribute the {@link #getInt(String)} method would be used.
 *
 * <p>
 * Note that attribute values are <b>not</b> converted when accessed. For example, it is
 * not possible to call {@link #getString(String)} if the underlying attribute is an
 * {@code int}. The only exception to this rule is {@code Class} and {@code Class[]}
 * values which may be accessed as {@code String} and {@code String[]} respectively to
 * prevent potential early class initialization.
 *
 * <p>
 * If necessary, a {@code MergedAnnotation} can be {@linkplain #synthesize() synthesized}
 * back into an actual {@link java.lang.annotation.Annotation}.
 *
 * @author Phillip Webb
...
public interface MergedAnnotation<A extends Annotation> {
	 ...
	 * {@linkplain #isDirectlyPresent() directly present} and {@linkplain #isMetaPresent()
	 * meta-present} annotations within the context of the {@link SearchStrategy} used.
	 ...
	 * <p>
	 * A directly present annotation is one that the user has explicitly declared and not
	 ...
	 * <p>
	 * A meta-present annotation is an annotation that the user hasn't explicitly
	 * declared, but has been used as a meta-annotation somewhere in the annotation
	 * hierarchy.
	 ...
	 * <p>
	 * A directly declared annotation has a distance of {@code 0}, a meta-annotation has a
	 * distance of {@code 1}, a meta-annotation on a meta-annotation has a distance of
	 * {@code 2}, etc. A {@linkplain #missing() missing} annotation will always return a
	 * distance of {@code -1}.
	 ...
	 * <p>
	 * Can be used to reorder a stream of annotations, for example, to give a higher
	 * priority to annotations declared on a superclass or interface. A
	 * {@linkplain #missing() missing} annotation will always return an aggregate index of
	 * {@code -1}.
	 * @return the aggregate index (starting at {@code 0}) or {@code -1} if the annotation
	 * is missing
	 */
	int getAggregateIndex();

	/**
	 * Get the source that ultimately declared the root annotation, or {@code null} if the
	 * source is not known.
	 * <p>
	 * If this merged annotation was created
	 * {@link MergedAnnotations#from(AnnotatedElement) from} an {@link AnnotatedElement}
	 * then this source will be an element of the same type. If the annotation was loaded
	 * without using reflection, the source can be of any type, but should have a sensible
	 * {@code toString()}. Meta-annotations will always return the same source as the
	 * {@link #getRoot() root}.
	...
	/**
	 * Get the source of the meta-annotation, or {@code null} if the annotation is not
	 * {@linkplain #isMetaPresent() meta-present}.
	 * <p>
	...
	/**
	 * Get the complete list of annotation types within the annotation hierarchy from this
	 * annotation to the {@link #getRoot() root}.
	 * <p>
	...
	/**
	 * Determine if the specified attribute name has a non-default value when compared to
	...
	/**
	 * Determine if the specified attribute name has a default value when compared to the
	 ...
	 * @param attributeName the attribute name
	 * @return an optional value or {@link Optional#empty()} if there is no matching
	 * attribute
	 */
	 ...
	 * Get an optional attribute value from the annotation.
	 * @param attributeName the attribute name
	 * @param type the attribute type. Must be compatible with the underlying attribute
	 * type or {@code Object.class}.
	 * @return an optional value or {@link Optional#empty()} if there is no matching
	 * attribute
	 */
	...
	/**
	 * Get the default attribute value from the annotation as specified in the annotation
	 * declaration.
	 * @param attributeName the attribute name
	 * @return an optional of the default value or {@link Optional#empty()} if there is no
	 ...
	 * removed.
	 * @return a filtered view of the annotation without any attributes that have a
	 * default value
	...
	/**
	 * Create a new view of the annotation with only attributes that match the given
	 * predicate.
	 ...
	 * <p>
	 * Methods from this view will return attribute values with only alias mirroring rules
	 * applied. Aliases to {@link #getMetaSource() meta-source} attributes will not be
	 * applied.
	...
	/**
	 * Create a new mutable {@link AnnotationAttributes} instance from this merged
	 * annotation.
	 * <p>
	...
	/**
	 * Create a type-safe synthesized version of this merged annotation that can be used
	 * directly in code.
	 * <p>
	 * The result is synthesized using a JDK {@link Proxy} and as a result may incur a
	 * computational cost when first invoked.
	 * <p>
	 * If this merged annotation was created {@linkplain #from(Annotation) from} an
	 * annotation instance, that annotation will be returned unmodified if it is not
	 * <em>synthesizable</em>. An annotation is considered synthesizable if one of the
	 * following is true.
	 * <ul>
	...
	/**
	 * Optionally create a type-safe synthesized version of this annotation based on a
	 * condition predicate.
	 * <p>
	 * The result is synthesized using a JDK {@link Proxy} and as a result may incur a
	 * computational cost when first invoked.
	 * <p>
	 * Consult the documentation for {@link #synthesize()} for an explanation of what is
	 * considered synthesizable.
	 * @param condition the test to determine if the annotation can be synthesized
	 * @return an optional containing the synthesized version of the annotation or an
	 * empty optional if the condition doesn't match
	 * @throws NoSuchElementException on a missing annotation
	 * @see MergedAnnotationPredicates
	 */
	...
	/**
	 * Create a {@link MergedAnnotation} that represents a missing annotation (i.e. one
	 * that is not present).
	 ...
	 * The resulting annotation will not have any attribute values but may still be used
	...
	/**
	 * Create a new {@link MergedAnnotation} instance of the specified annotation type
	 * with attribute values supplied by a map.
	 * @param annotationType the annotation type
	 * @param attributes the annotation attributes or {@code null} if just default values
	 * should be used
	...
	enum Adapt {
		...
		/**
		 * Adapt nested annotation or annotation arrays to maps rather than synthesizing
		 * the values.
		 */
		ANNOTATION_TO_MAP;
