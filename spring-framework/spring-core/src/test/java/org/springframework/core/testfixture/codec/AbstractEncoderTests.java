 ...
 * {@link #canEncode()} and {@link #encode()}, possibly using the wide * variety of helper
 * methods like {@link #testEncodeAll}.
 *
...
public abstract class AbstractEncoderTests<E extends Encoder<?>> extends AbstractLeakCheckingTests {
	...
	/**
	 * Subclasses should implement this method to test {@link Encoder#encode}, possibly
	 * using {@link #testEncodeAll} or other helper methods.
	 */
	@Test
	...
	protected <T> void testEncodeAll(Publisher<? extends T> input, ResolvableType inputType,
			Consumer<StepVerifier.FirstStep<DataBuffer>> stepConsumer, @Nullable MimeType mimeType,
	...
	/**
	 * Test a {@link Encoder#encode encode} scenario where the input stream contains an
	 * error. This test method will feed the first element of the {@code input} stream to
	 * the encoder, followed by an {@link InputException}. The result is expected to
	 ...
	 * @see InputException
	 */
	protected void testEncodeError(Publisher<?> input, ResolvableType inputType, @Nullable MimeType mimeType,
			@Nullable Map<String, Object> hints) {
		...
		StepVerifier.create(result).consumeNextWith(DataBufferUtils::release).expectError(InputException.class)
				.verify();
	}
	 ...
	 * @param hints the hints used for decoding. May be {@code null}.
	 */
	protected void testEncodeCancel(Publisher<?> input, ResolvableType inputType, @Nullable MimeType mimeType,
			@Nullable Map<String, Object> hints) {
	...
	/**
	 * Test a {@link Encoder#encode encode} scenario where the input stream is empty. The
	 ...
	 * @return a consumer that expects the given data buffer to be equal to
	 * {@code expected}
	 */
