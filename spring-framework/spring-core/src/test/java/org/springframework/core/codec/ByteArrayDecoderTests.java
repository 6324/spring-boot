...
class ByteArrayDecoderTests extends AbstractDecoderTests<ByteArrayDecoder> {
	...
	public void canDecode() {
		...
		assertThat(this.decoder.canDecode(ResolvableType.forClass(byte[].class), MimeTypeUtils.APPLICATION_JSON))
				.isTrue();
	}
	...
	public void decode() {
		...
		testDecodeAll(input, byte[].class, step -> step.consumeNextWith(expectBytes(this.fooBytes))
