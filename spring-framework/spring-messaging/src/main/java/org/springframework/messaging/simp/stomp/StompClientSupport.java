 ...
 *
 * <p>
 * Subclasses can connect over WebSocket or TCP using any library. When creating a new
 * connection, a subclass can create an instance of @link DefaultStompSession} which
 * extends {@link org.springframework.messaging.tcp.TcpConnectionHandler} whose lifecycle
 * methods the subclass must then invoke.
 *
 * <p>
 * In effect, {@code TcpConnectionHandler} and {@code TcpConnection} are the contracts
 * that any subclass must adapt to while using {@link StompEncoder} and
...
public abstract class StompClientSupport {
	 ...
	 * outgoing messages to and from {@code byte[]} based on object type and the
	 * "content-type" header.
	 * <p>
	 ...
	 * <p>
	 * <strong>Note:</strong> Some transports have built-in support to work with
	 * heartbeats and therefore do not require a TaskScheduler. Receipts however, if
	 * needed, do require a TaskScheduler to be configured.
	 * <p>
	 ...
	 * The first number represents how often the client will write or send a heart-beat.
	 * The second is how often the server should write. A value of 0 means no heart-beats.
	 * <p>
	 * By default this is set to "10000,10000" but subclasses may override that default
	 * and for example set it to "0,0" if they require a TaskScheduler to be configured
	 * first.
	 ...
	 * <p>
	 * Returns {@code false} if {@link #setDefaultHeartbeat defaultHeartbeat} is set to
	 * "0,0", and {@code true} otherwise.
	 */
	 ...
	 */
	protected ConnectionHandlingStompSession createSession(@Nullable StompHeaders connectHeaders,
	...
	/**
	 * Further initialize the StompHeaders, for example setting the heart-beat header if
	 * necessary.
