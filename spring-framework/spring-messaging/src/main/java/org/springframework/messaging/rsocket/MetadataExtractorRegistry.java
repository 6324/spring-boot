...
/**
 * Stores registrations of extractors for metadata entries. Each metadata entry is decoded
 * based on its {@code MimeType} and a name is assigned to the decoded value.
 *
...
public interface MetadataExtractorRegistry {
	...
	/**
	 * Decode metadata entries with the given {@link MimeType} to the specified target
	 * class, and store the decoded value in the output map under the given name.
	 * @param mimeType the mime type of metadata entries to extract
	 * @param targetType the target value type to decode to
	 * @param name assign a name for the decoded value; if not provided, then the mime
	 ...
	 * {@link ParameterizedTypeReference} instead of {@link Class} for specifying a target
	...
	/**
	 * Variant of {@link #metadataToExtract(MimeType, Class, String)} that allows custom
	 * logic to be used to map the decoded value to any number of values in the output
	 * map.
	 * @param mimeType the mime type of metadata entries to extract
	 * @param targetType the target value type to decode to
	...
	/**
	 * Variant of {@link #metadataToExtract(MimeType, Class, BiConsumer)} that accepts
	 * {@link ParameterizedTypeReference} instead of {@link Class} for specifying a target
	 * type with generic parameters.
	 * @param mimeType the mime type of metadata entries to extract
	 ...
	 * @param <T> the target value type
	 */
	<T> void metadataToExtract(MimeType mimeType, ParameterizedTypeReference<T> type,
