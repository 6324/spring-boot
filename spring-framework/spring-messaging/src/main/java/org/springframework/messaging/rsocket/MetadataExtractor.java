...
/**
 * Strategy to extract a map of value(s) from {@link Payload} metadata, which could be
 * composite metadata with multiple entries. Each metadata entry is decoded based on its
 * {@code MimeType} and a name is assigned to the decoded value. The resulting name-value
 * pairs can be added to the headers of a {@link org.springframework.messaging.Message
 * Message}.
 *
...
public interface MetadataExtractor {
	...
	/**
	 * Extract a map of values from the given {@link Payload} metadata. The Payload
	 * "route", if present, should be saved under {@link #ROUTE_KEY}.
