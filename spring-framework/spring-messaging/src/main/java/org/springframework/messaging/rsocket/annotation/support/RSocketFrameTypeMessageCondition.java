...
/**
 * A condition to assist with mapping onto handler methods based on the RSocket frame
 * type. This helps to separate the handling of connection-level frame types, i.e.
 * {@code SETUP} and {@code METADATA_PUSH}, from the handling of stream requests.
 *
...
public class RSocketFrameTypeMessageCondition extends AbstractMessageCondition<RSocketFrameTypeMessageCondition> {
	...
	public static final RSocketFrameTypeMessageCondition CONNECT_CONDITION = new RSocketFrameTypeMessageCondition(
			FrameType.SETUP, FrameType.METADATA_PUSH);

	/** Match RSocket frames "REQUEST_FNF" or "REQUEST_RESPONSE". */
	public static final RSocketFrameTypeMessageCondition REQUEST_FNF_OR_RESPONSE_CONDITION = new RSocketFrameTypeMessageCondition(
			FrameType.REQUEST_FNF, FrameType.REQUEST_RESPONSE);

	/** Match RSocket frame "REQUEST_RESPONSE". */
	public static final RSocketFrameTypeMessageCondition REQUEST_RESPONSE_CONDITION = new RSocketFrameTypeMessageCondition(
			FrameType.REQUEST_RESPONSE);

	/** Match RSocket frame "REQUEST_STREAM". */
	public static final RSocketFrameTypeMessageCondition REQUEST_STREAM_CONDITION = new RSocketFrameTypeMessageCondition(
			FrameType.REQUEST_STREAM);

	/** Match RSocket frame "REQUEST_CHANNEL". */
	public static final RSocketFrameTypeMessageCondition REQUEST_CHANNEL_CONDITION = new RSocketFrameTypeMessageCondition(
			FrameType.REQUEST_CHANNEL);

	/**
	 * Empty condition that does not match to any RSocket frames (e.g. for type-level
	 * mappings)
	 */
	public static final RSocketFrameTypeMessageCondition EMPTY_CONDITION = new RSocketFrameTypeMessageCondition();

	/**
	 * Condition to match "REQUEST_FNF", "REQUEST_RESPONSE", "REQUEST_STREAM", and
	 * "REQUEST_CHANNEL".
	 * @deprecated as of 5.2.2 because matching to all interaction types is too flexible.
	 * Please use one of the other constants in this class that match to specific frames.
	 */
	@Deprecated
	public static final RSocketFrameTypeMessageCondition REQUEST_CONDITION = new RSocketFrameTypeMessageCondition(
			FrameType.REQUEST_FNF, FrameType.REQUEST_RESPONSE, FrameType.REQUEST_STREAM, FrameType.REQUEST_CHANNEL);
	...
	/**
	 * Return a condition for matching the RSocket request interaction type with that is
	 * selected based on the delcared request and response cardinality of some handler
	 * method.
	 * <p>
	 ...
	 * </tr>
	 * </table>
	 * @param cardinalityIn -- the request cardinality: 1 for a single payload, 2 for many
	 * payloads, and 0 if input is not handled.
	 * @param cardinalityOut -- the response cardinality: 0 for no output payloads, 1 for
	 * a single payload, and 2 for many payloads.
