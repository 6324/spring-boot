 ...
 * interface based on a JPA {@code ClassTransformer} which a JPA PersistenceProvider asks
 * the {@code PersistenceUnitInfo} to install in the current runtime.
 *
 * @author Rod Johnson
	...
	public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined,
		...
		synchronized (this) {
			if (this.currentlyTransforming) {
				// Defensively back out when called from within the transform delegate
				// below:
				// in particular, for the over-eager transformer implementation in
				// Hibernate 5.
				return null;
			}
