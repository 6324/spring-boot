...
/**
 * Abstract {@link JpaVendorAdapter} implementation that defines common properties, to be
 * translated into vendor-specific JPA properties by concrete subclasses.
 *
...
public abstract class AbstractJpaVendorAdapter implements JpaVendorAdapter {
	 ...
	 * Custom vendor properties may still fine-tune the database dialect. However, there
	 * may nevertheless be conflicts: For example, specify either this setting or
	 * Hibernate's "hibernate.dialect_resolvers" property, not both.
	 */
	 ...
	 * <p>
	 * Note that the exact semantics of this flag depend on the underlying persistence
	 * provider. For any more advanced needs, specify the appropriate vendor-specific
	 * settings as "jpaProperties".
	 * <p>
	 * <b>NOTE: Do not set this flag to 'true' while also setting JPA 2.1's
	 * {@code javax.persistence.schema-generation.database.action} property.</b> These two
	 * schema generation mechanisms - standard JPA versus provider-native - are mutually
	 * exclusive, e.g. with Hibernate 5.
	 ...
	 * <p>
	 * For more specific logging configuration, specify the appropriate vendor-specific
	 * settings as "jpaProperties".
	 * @see org.springframework.orm.jpa.AbstractEntityManagerFactoryBean#setJpaProperties
	 */
