...
/**
 * Abstract {@link org.springframework.beans.factory.FactoryBean} that creates a local JPA
 * {@link javax.persistence.EntityManagerFactory} instance within a Spring application
 * context.
 *
 * <p>
 * Encapsulates the common functionality between the different JPA bootstrap contracts
 * (standalone as well as container).
 *
 * <p>
 * Implements support for standard JPA configuration conventions as well as Spring's
 * customizable {@link JpaVendorAdapter} mechanism, and controls the
 * EntityManagerFactory's lifecycle.
 *
 * <p>
 * This class also implements the
 * {@link org.springframework.dao.support.PersistenceExceptionTranslator} interface, as
 * autodetected by Spring's
 * {@link org.springframework.dao.annotation.PersistenceExceptionTranslationPostProcessor},
 * for AOP-based translation of native exceptions to Spring DataAccessExceptions. Hence,
 * the presence of e.g. LocalEntityManagerFactoryBean automatically enables a
...
public abstract class AbstractEntityManagerFactoryBean
		implements FactoryBean<EntityManagerFactory>, BeanClassLoaderAware, BeanFactoryAware, BeanNameAware,
	 ...
	 * EntityManagerFactory. If not specified, the persistence provider will be taken from
	 * the JpaVendorAdapter (if any) or retrieved through scanning (as far as possible).
	 ...
	 * (if any) or determined by the persistence unit deployment descriptor (as far as
	 * possible).
	 * @see JpaVendorAdapter#getPersistenceProvider()
	 * @see javax.persistence.spi.PersistenceProvider
	 * @see javax.persistence.Persistence
	 ...
	 * <p>
	 * Default is none, indicating the default EntityManagerFactory configuration. The
	 * persistence provider will throw an exception if ambiguous EntityManager
	 ...
	 * <p>
	 * The default will be taken from the specific JpaVendorAdapter, if any, or set to the
	 * standard {@code javax.persistence.EntityManagerFactory} interface else.
	...
	/**
	 * Specify the (potentially vendor-specific) EntityManager interface that this
	 * factory's EntityManagers are supposed to implement.
	 * <p>
	 * The default will be taken from the specific JpaVendorAdapter, if any, or set to the
	 * standard {@code javax.persistence.EntityManager} interface else.
	 ...
	 * EntityManagerFactory. This will be exposed through the EntityManagerFactoryInfo
	 * interface, to be picked up as default dialect by accessors that intend to use
	 * JpaDialect functionality.
	 ...
	 * This will initialize appropriate defaults for the given provider, such as
	 * persistence provider class and JpaDialect, unless locally overridden in this
	 * FactoryBean.
	 */
	 ...
	 * injection purposes instead of waiting for the JPA provider's bootstrapping to
	 * complete. However, note that the first actual call to a
	 * {@code EntityManagerFactory} method will then block until the JPA provider's
	 * bootstrapping completed, if not ready by then. For maximum benefit, make sure to
	 * avoid early {@code EntityManagerFactory} calls in init methods of related beans,
	 * even for metadata introspection purposes.
	 * @since 4.3
	 */
	 ...
	 * @param emf the EntityManagerFactory as returned by the persistence provider, if
	 * initialized already
	...
	/**
	 * Delegate an incoming invocation from the proxy, dispatching to
	 * EntityManagerFactoryInfo or the native EntityManagerFactory accordingly.
	 */
	Object invokeProxyMethod(Method method, @Nullable Object[] args) throws Throwable {
		if (method.getDeclaringClass().isAssignableFrom(EntityManagerFactoryInfo.class)) {
			...
			// Redirect to plain createEntityManager and add synchronization semantics
		...
		// Standard delegation to the native factory, just post-processing EntityManager
		// return values
		Object retVal = method.invoke(getNativeEntityManagerFactory(), args);
		if (retVal instanceof EntityManager) {
			// Any other createEntityManager variant - expecting non-synchronized
			// semantics
	...
	/**
	 * Subclasses must implement this method to create the EntityManagerFactory that will
	 * be returned by the {@code getObject()} method.
	...
	/**
	 * Implementation of the PersistenceExceptionTranslator interface, as autodetected by
	 * Spring's PersistenceExceptionTranslationPostProcessor.
	 * <p>
	 * Uses the dialect's conversion if possible; otherwise falls back to standard JPA
		...
		if (this.nativeEntityManagerFactory != null) {
			...
			try {
				...
				throw new IllegalStateException(
						"Failed to asynchronously initialize native EntityManagerFactory: " + ex.getMessage(), cause);
			}
		}
	}
	 ...
	 * Resolved to the actual AbstractEntityManagerFactoryBean instance on
	 * deserialization.
	 */
	@SuppressWarnings("serial")
	 ...
	 * proxy {@link EntityManager} (if necessary) from {@code createEntityManager}
	 * methods.
	 */
	@SuppressWarnings("serial")
