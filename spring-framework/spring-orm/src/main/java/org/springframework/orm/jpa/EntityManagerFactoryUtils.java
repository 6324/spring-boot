 ...
 * EntityManager instances within transactions. Also provides support for exception
 * translation.
 *
 * <p>
...
public abstract class EntityManagerFactoryUtils {
	 ...
	 * EntityManagers. Return DataSourceUtils.CONNECTION_SYNCHRONIZATION_ORDER - 100 to
	 * execute EntityManager cleanup before JDBC Connection cleanup, if any.
	...
	/**
	 * Find an EntityManagerFactory with the given name in the given Spring application
	 * context (represented as ListableBeanFactory).
	 * <p>
	 * The specified unit name will be matched against the configured persistence unit,
	 * provided that a discovered EntityManagerFactory implements the
	 * {@link EntityManagerFactoryInfo} interface. If not, the persistence unit name will
	 * be matched against the Spring bean name, assuming that the EntityManagerFactory
	 * bean names follow that convention.
	 * <p>
	 ...
	 * @param unitName the name of the persistence unit (may be {@code null} or empty, in
	 * which case a single bean of type EntityManagerFactory will be searched for)
	...
	public static EntityManagerFactory findEntityManagerFactory(ListableBeanFactory beanFactory,
		...
		if (StringUtils.hasLength(unitName)) {
			// See whether we can find an EntityManagerFactory with matching persistence
			// unit name.
			...
			// Find unique EntityManagerFactory bean in the context, falling back to
			// parent contexts.
	 ...
	 * @throws DataAccessResourceFailureException if the EntityManager couldn't be
	 * obtained
	 ...
	 * call (may be {@code null})
	 * @return the EntityManager, or {@code null} if none found
	 * @throws javax.persistence.PersistenceException if the EntityManager couldn't be
	 * created
	...
	public static EntityManager doGetTransactionalEntityManager(EntityManagerFactory emf,
		...
		if (emHolder != null) {
			if (synchronizedWithTransaction) {
				...
				// Use holder's reference count to track synchronizedWithTransaction
				// access.
				...
				if (emHolder.isTransactionActive() && !emHolder.isOpen()) {
					...
					// EntityManagerHolder with an active transaction coming from
					// JpaTransactionManager,
					// with no synchronized EntityManager having been requested by
					// application code before.
					// Unbind in order to register a new unsynchronized EntityManager
					// instead.
					TransactionSynchronizationManager.unbindResource(emf);
				}
				else {
					// Either a previously bound unsynchronized EntityManager, or the
					// application
					// has requested a synchronized EntityManager before and therefore
					// upgraded
		...
		if (!synchronizedWithTransaction) {
			try {
				...
				// JPA 2.1 API available but method not actually implemented in
		...
		try {
			...
			// Thread-bound object will get removed by synchronization at transaction
			// completion.
			...
			if (synchronizedWithTransaction) {
				...
				// Unsynchronized - just scope it for the transaction, as demanded by the
				// JPA 2.1 spec...
			...
			// Unexpected exception from external delegation call -> close EntityManager
			// and rethrow.
	 ...
	 * <p>
	 * This method sets the JPA 2.0 query hint "javax.persistence.query.timeout"
	 * accordingly.
	 ...
	 * {@code org.springframework.dao} hierarchy. Return null if no translation is
	 * appropriate: any other exception may have resulted from user code, and should not
	 * be translated.
	 * <p>
	 * The most important cases like object not found or optimistic locking failure are
	 * covered here. For more fine-granular conversion, JpaTransactionManager etc support
	 * sophisticated translation of exceptions via a JpaDialect.
	 * @param ex runtime exception that occurred
	 * @return the corresponding DataAccessException instance, or {@code null} if the
	...
	/**
	 * Close the given JPA EntityManager, catching and logging any cleanup exceptions
	 * thrown.
	...
	/**
	 * Callback for resource cleanup at the end of a non-JPA transaction (e.g. when
	 * participating in a JtaTransactionManager transaction), fully synchronized with the
	 * ongoing transaction.
	 *
	 * @see org.springframework.transaction.jta.JtaTransactionManager
	 */
	private static class TransactionalEntityManagerSynchronization
			extends ResourceHolderSynchronization<EntityManagerHolder, EntityManagerFactory> implements Ordered {
		...
		public TransactionalEntityManagerSynchronization(EntityManagerHolder emHolder, EntityManagerFactory emf,
				@Nullable Object txData, boolean newEm) {
		...
		protected void flushResource(EntityManagerHolder resourceHolder) {
			...
			if (em instanceof EntityManagerProxy) {
				...
				if (TransactionSynchronizationManager.hasResource(target)) {
					// ExtendedEntityManagerSynchronization active after joinTransaction()
					// call:
		...
		@Override
		protected void cleanupResource(EntityManagerHolder resourceHolder, EntityManagerFactory resourceKey,
				boolean committed) {
