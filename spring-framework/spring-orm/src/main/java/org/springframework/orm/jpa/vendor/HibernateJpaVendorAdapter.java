 ...
 *
 * <p>
 * Exposes Hibernate's persistence provider and EntityManager extension interface, and
 * adapts {@link AbstractJpaVendorAdapter}'s common configuration settings. Also supports
 ...
 *
 * <p>
 * <b>A note about {@code HibernateJpaVendorAdapter} vs native Hibernate settings:</b>
 * Some settings on this adapter may conflict with native Hibernate configuration rules or
 * custom Hibernate properties. For example, specify either {@link #setDatabase} or
...
public class HibernateJpaVendorAdapter extends AbstractJpaVendorAdapter {
	 ...
	 * <p>
	 * See {@link HibernateJpaDialect#setPrepareConnection(boolean)} for details. This is
	 * just a convenience flag passed through to {@code HibernateJpaDialect}.
	 * <p>
	 * On Hibernate 5.1+, this flag remains {@code true} by default like against previous
	 * Hibernate versions. The vendor adapter manually enforces Hibernate's new connection
	 * handling mode {@code DELAYED_ACQUISITION_AND_HOLD} in that case unless a
	 * user-specified connection handling mode property indicates otherwise; switch this
	 * flag to {@code false} to avoid that interference.
	 * <p>
	 * <b>NOTE: For a persistence unit with transaction type JTA e.g. on WebLogic, the
	 * connection release mode will never be altered from its provider default, i.e. not
	 * be forced to {@code DELAYED_ACQUISITION_AND_HOLD} by this flag.</b> Alternatively,
	...
	public Map<String, Object> getJpaPropertyMap(PersistenceUnitInfo pui) {
		return buildJpaPropertyMap(
				this.jpaDialect.prepareConnection && pui.getTransactionType() != PersistenceUnitTransactionType.JTA);
	}
	...
	private Map<String, Object> buildJpaPropertyMap(boolean connectionReleaseOnClose) {
		...
		if (connectionReleaseOnClose) {
			// Hibernate 5.1+: manually enforce connection release mode ON_CLOSE (the
			// former default)
			try {
				// Try Hibernate 5.2+
