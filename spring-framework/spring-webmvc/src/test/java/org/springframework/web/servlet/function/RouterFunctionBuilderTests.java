...
public class RouterFunctionBuilderTests {
	...
	public void route() {
		...
		Optional<Integer> responseStatus = route.route(getFooRequest)
				.map(handlerFunction -> handle(handlerFunction, getFooRequest)).map(ServerResponse::statusCode)
		...
		responseStatus = route.route(headFooRequest).map(handlerFunction -> handle(handlerFunction, getFooRequest))
		...
		responseStatus = route.route(barRequest).map(handlerFunction -> handle(handlerFunction, barRequest))
				.map(ServerResponse::statusCode).map(HttpStatus::value);
		...
		responseStatus = route.route(invalidRequest).map(handlerFunction -> handle(handlerFunction, invalidRequest))
				.map(ServerResponse::statusCode).map(HttpStatus::value);
	...
	public void resources() {
		...
		MockHttpServletRequest servletRequest = new MockHttpServletRequest("GET", "/resources/response.txt");
		...
		Optional<Integer> responseStatus = route.route(resourceRequest)
				.map(handlerFunction -> handle(handlerFunction, resourceRequest)).map(ServerResponse::statusCode)
				.map(HttpStatus::value);
		assertThat(responseStatus.get().intValue()).isEqualTo(200);
	...
	public void nest() {
		...
		Optional<Integer> responseStatus = route.route(fooRequest)
				.map(handlerFunction -> handle(handlerFunction, fooRequest)).map(ServerResponse::statusCode)
				.map(HttpStatus::value);
		assertThat(responseStatus.get().intValue()).isEqualTo(200);
	}
	...
	public void filters() {
		...
		RouterFunction<ServerResponse> route = RouterFunctions.route()
				.GET("/foo", request -> ServerResponse.ok().build()).GET("/bar", request -> {
		...
		Optional<Integer> responseStatus = route.route(barRequest)
				.map(handlerFunction -> handle(handlerFunction, barRequest)).map(ServerResponse::statusCode)
				.map(HttpStatus::value);
