...
/**
 * Spring MVC {@link View} that renders JSON content by serializing the model for the
 * current request using <a href="https://github.com/FasterXML/jackson">Jackson 2's</a>
 * {@link ObjectMapper}.
 *
 * <p>
 * By default, the entire contents of the model map (with the exception of
 * framework-specific classes) will be encoded as JSON. If the model contains only one
 * key, you can have it extracted encoded as JSON alone via
 * {@link #setExtractValueFromSingleKeyModel}.
 *
 * <p>
...
public class MappingJackson2JsonView extends AbstractJackson2View {
	 ...
	 * provided by {@link Jackson2ObjectMapperBuilder} and setting the content type to
	...
	/**
	 * Set the attributes in the model that should be rendered by this view. When set, all
	...
	/**
	 * Set whether to serialize models containing a single attribute as a map or whether
	 * to extract the single value from the model and serialize it directly.
	 * <p>
	 ...
	 * <p>
	 * The default implementation removes {@link BindingResult} instances and entries not
	 * included in the {@link #setModelKeys modelKeys} property.
	...
	protected Object filterModel(Map<String, Object> model) {
		...
		model.forEach((clazz, value) -> {
			if (!(value instanceof BindingResult) && modelKeys.contains(clazz)
					&& !clazz.equals(JsonView.class.getName()) && !clazz.equals(FilterProvider.class.getName())) {
