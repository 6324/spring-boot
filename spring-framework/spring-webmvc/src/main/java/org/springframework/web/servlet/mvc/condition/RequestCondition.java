 ...
 *
 * <p>
 * Request conditions can be combined via {@link #combine(Object)}, matched to a request
 * via {@link #getMatchingCondition(HttpServletRequest)}, and compared to each other via
 * {@link #compareTo(Object, HttpServletRequest)} to determine which is a closer match for
 * a given request.
 *
...
public interface RequestCondition<T> {
	 ...
	 * <p>
	 * For CORS pre-flight requests, conditions should match to the would-be, actual
	 * request (e.g. URL pattern, query parameters, and the HTTP method from the
	 * "Access-Control-Request-Method" header). If a condition cannot be matched to a
	 * pre-flight request it should return an instance with empty content thus not causing
	 * a failure to match.
	 ...
	 * {@link #getMatchingCondition(HttpServletRequest)} to ensure they have content
