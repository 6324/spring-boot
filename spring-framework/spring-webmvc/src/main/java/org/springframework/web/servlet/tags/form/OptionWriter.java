...
/**
 * Provides supporting functionality to render a list of '{@code option}' tags based on
 * some source object. This object can be either an array, a {@link Collection}, or a
 * {@link Map}.
 * <h3>Using an array or a {@link Collection}:</h3>
 * <p>
 * If you supply an array or {@link Collection} source object to render the inner
 * '{@code option}' tags, you may optionally specify the name of the property on the
 * objects which corresponds to the <em>value</em> of the rendered '{@code option}' (i.e.,
 * the {@code valueProperty}) and the name of the property that corresponds to the
 * <em>label</em> (i.e., the {@code labelProperty}). These properties are then used when
 * rendering each element of the array/{@link Collection} as an '{@code option}'. If
 * either property name is omitted, the value of {@link Object#toString()} of the
 * corresponding array/{@link Collection} element is used instead. However, if the item is
 * an enum, {@link Enum#name()} is used as the default value.
 * </p>
 * <h3>Using a {@link Map}:</h3>
 * <p>
 * You can alternatively choose to render '{@code option}' tags by supplying a {@link Map}
 * as the source object.
 * </p>
 * <p>
 * If you <strong>omit</strong> property names for the <em>value</em> and <em>label</em>:
 * </p>
 * <ul>
 * <li>the {@code key} of each {@link Map} entry will correspond to the <em>value</em> of
 * the rendered '{@code option}', and</li>
 * <li>the {@code value} of each {@link Map} entry will correspond to the <em>label</em>
 * of the rendered '{@code option}'.</li>
 * </ul>
 * <p>
 ...
 * </p>
 * <ul>
 * <li>the <em>value</em> of the rendered '{@code option}' will be retrieved from the
 * {@code valueProperty} on the object corresponding to the {@code key} of each
 * {@link Map} entry, and</li>
 * <li>the <em>label</em> of the rendered '{@code option}' will be retrieved from the
 * {@code labelProperty} on the object corresponding to the {@code value} of each
 * {@link Map} entry.
 * </ul>
 * <h3>When using either of these approaches:</h3>
 * <ul>
 * <li>Property names for the <em>value</em> and <em>label</em> are specified as arguments
 * to the {@link #OptionWriter(Object, BindStatus, String, String, boolean)
 * constructor}.</li>
 * <li>An '{@code option}' is marked as 'selected' if its key {@link #isOptionSelected
 * matches} the value that is bound to the tag instance.</li>
 * </ul>
 *
 * @author Rob Harrop
...
class OptionWriter {
	 ...
	 * (optional)
	 */
	public OptionWriter(Object optionSource, BindStatus bindStatus, @Nullable String valueProperty,
			@Nullable String labelProperty, boolean htmlEscape) {
	...
	/**
	 * Write the '{@code option}' tags for the configured {@link #optionSource} to the
	 * supplied {@link TagWriter}.
	 */
	...
	/**
	 * Render the inner '{@code option}' tags using the supplied {@link Map} as the
	 * source.
	 ...
	 * objects as the source. The value of the {@link #valueProperty} field is used when
	 * rendering the '{@code value}' of the '{@code option}' and the value of the
	...
	/**
	 * Render an HTML '{@code option}' with the supplied value and label. Marks the value
	 * as 'selected' if either the item itself or its value match the bound value.
	 */
	...
	/**
	 * Determine the display value of the supplied {@code Object}, HTML-escaped as
	 * required.
	 */
