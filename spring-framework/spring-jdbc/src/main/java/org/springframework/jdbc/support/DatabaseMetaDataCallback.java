...
/**
 * A callback interface used by the JdbcUtils class. Implementations of this interface
 * perform the actual work of extracting database meta-data, but don't need to worry about
 * exception handling. SQLExceptions will be caught and handled correctly by the JdbcUtils
 * class.
 *
...
public interface DatabaseMetaDataCallback<T> {
	 ...
	 * @return a result object extracted from the meta-data (can be an arbitrary object,
	 ...
	 * @throws MetaDataAccessException in case of other failures while extracting
	 * meta-data (for example, reflection failure)
	 */
