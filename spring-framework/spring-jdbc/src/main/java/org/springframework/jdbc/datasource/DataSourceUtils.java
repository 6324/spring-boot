 ...
 * {@link javax.sql.DataSource}. Includes special support for Spring-managed transactional
 * Connections, e.g. managed by {@link DataSourceTransactionManager} or
 * {@link org.springframework.transaction.jta.JtaTransactionManager}.
 *
 * <p>
 * Used internally by Spring's {@link org.springframework.jdbc.core.JdbcTemplate},
 * Spring's JDBC operation objects and the JDBC {@link DataSourceTransactionManager}. Can
 * also be used directly in application code.
 *
 * @author Rod Johnson
...
public abstract class DataSourceUtils {
	 ...
	 * Spring hierarchy of unchecked generic data access exceptions, simplifying calling
	 * code and making any exception that is thrown more meaningful.
	 * <p>
	 ...
	 * @throws org.springframework.jdbc.CannotGetJdbcConnectionException if the attempt to
	...
	public static Connection doGetConnection(DataSource dataSource) throws SQLException {
		...
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			try {
				...
				// Unexpected exception from external delegation call -> close Connection
				// and rethrow.
	...
	/**
	 * Actually fetch a {@link Connection} from the given {@link DataSource}, defensively
	 * turning an unexpected {@code null} return value from
			...
			try {
				...
				while (exToCheck != null) {
					if (exToCheck.getClass().getSimpleName().contains("Timeout")) {
						// Assume it's a connection timeout that would otherwise get lost:
						// e.g. from JDBC 4.0
						throw ex;
					}
					exToCheck = exToCheck.getCause();
				}
				// "read-only not supported" SQLException -> ignore, it's just a hint
				// anyway
	...
	public static void resetConnectionAfterTransaction(Connection con, @Nullable Integer previousIsolationLevel,
		...
		boolean debugEnabled = logger.isDebugEnabled();
		try {
			// Reset transaction isolation to previous value, if changed for the
			// transaction.
			...
			// Reset read-only flag if we originally switched it to true on transaction
			// begin.
			if (resetReadOnly) {
				if (debugEnabled) {
	...
	/**
	 * Determine whether the given JDBC Connection is transactional, that is, bound to the
	...
	/**
	 * Apply the specified timeout - overridden by the current transaction timeout, if any
	 ...
	 * @param con the Connection to close if necessary (if this is {@code null}, the call
	 * will be ignored)
	 * @param dataSource the DataSource that the Connection was obtained from (may be
	 * {@code null})
	 * @see #getConnection
	 ...
	 * {@link #releaseConnection}, but throwing the original SQLException.
	 * <p>
	 * Directly accessed by {@link TransactionAwareDataSourceProxy}.
	 * @param con the Connection to close if necessary (if this is {@code null}, the call
	 * will be ignored)
	...
	/**
	 * Determine whether the given two Connections are equal, asking the target Connection
	 * in case of a proxy. Used to detect equality even if the user passed in a raw target
	 ...
	 * Connection is a proxy, it will be unwrapped until a non-proxy Connection is found.
	 ...
	 * Decreased for every level of nesting that a DataSource has, checked through the
	 * level of DelegatingDataSource nesting.
