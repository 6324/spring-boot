 ...
 * Spring-managed transactions. Similar to a transactional JNDI DataSource as provided by
 * a Java EE server.
 *
 * <p>
 ...
 * transaction manager, for example {@link DataSourceTransactionManager}, still needs to
 * work with the underlying DataSource, <i>not</i> with this proxy.
 *
 * <p>
 * <b>Make sure that TransactionAwareDataSourceProxy is the outermost DataSource of a
 * chain of DataSource proxies/adapters.</b> TransactionAwareDataSourceProxy can delegate
 * either directly to the target connection pool or to some intermediary proxy/adapter
 * like {@link LazyConnectionDataSourceProxy} or {@link UserCredentialsDataSourceAdapter}.
 *
 * <p>
 * Delegates to {@link DataSourceUtils} for automatically participating in thread-bound
 ...
 * not within a transaction, normal DataSource behavior applies.
 *
 * <p>
 ...
 * environment. However, if possible, use Spring's DataSourceUtils, JdbcTemplate or JDBC
 * operation objects to get transaction participation even without a proxy for the target
 * DataSource, avoiding the need to define such a proxy in the first place.
 *
 * <p>
 * As a further effect, using a transaction-aware DataSource will apply remaining
 * transaction timeouts to all created JDBC (Prepared/Callable)Statement. This means that
 * all operations performed through standard JDBC will automatically participate in
 * Spring-managed transaction timeouts.
 *
 * <p>
 * <b>NOTE:</b> This DataSource proxy needs to return wrapped Connections (which implement
 * the {@link ConnectionProxy} interface) in order to handle close calls properly. Use
 * {@link Connection#unwrap} to retrieve the native JDBC Connection.
 *
...
public class TransactionAwareDataSourceProxy extends DelegatingDataSource {
	...
	/**
	 * Specify whether to reobtain the target Connection for each operation performed
	 * within a transaction.
	 * <p>
	 * The default is "false". Specify "true" to reobtain transactional Connections for
	 * every call on the Connection proxy; this is advisable on JBoss if you hold on to a
	 * Connection handle across transaction boundaries.
	 * <p>
	 * The effect of this setting is similar to the "hibernate.connection.release_mode"
	...
	/**
	 * Wraps the given Connection with a proxy that delegates every method call to it but
	 * delegates {@code close()} calls to DataSourceUtils.
	...
	/**
	 * Determine whether to obtain a fixed target Connection for the proxy or to reobtain
	 * the target Connection for each operation.
	 * <p>
	 * The default implementation returns {@code true} for all standard cases. This can be
	 * overridden through the {@link #setReobtainTransactionalConnections
	 * "reobtainTransactionalConnections"} flag, which enforces a non-fixed target
	 * Connection within an active transaction. Note that non-transactional access will
	 * always use a fixed Connection.
	 * @param targetDataSource the target DataSource
	 */
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			...
			if (this.target == null) {
				if (method.getName().equals("getWarnings") || method.getName().equals("clearWarnings")) {
					// Avoid creation of target Connection on pre-close cleanup (e.g.
					// Hibernate Session)
					return null;
				}
				if (this.closed) {
