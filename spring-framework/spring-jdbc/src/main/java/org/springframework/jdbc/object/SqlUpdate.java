 ...
 *
 * <p>
 * This class provides a number of {@code update} methods, analogous to the
 * {@code execute} methods of query objects.
 *
 * <p>
 * This class is concrete. Although it can be subclassed (for example to add a custom
 * update method) it can easily be parameterized by setting SQL and declaring parameters.
 *
 * <p>
...
public class SqlUpdate extends SqlOperation {
	...
	/**
	 * Maximum number of rows the update may affect. If more are affected, an exception
	 * will be thrown. Ignored if 0.
	 */
	 ...
	 * @param types the SQL types of the parameters, as defined in the
	 * {@code java.sql.Types} class
	 * @param maxRowsAffected the maximum number of rows that may be affected by the
	 * update
	 * @see java.sql.Types
	 */
	...
	/**
	 * Set the maximum number of rows that may be affected by this update. The default
	 * value is 0, which does not limit the number of rows affected.
	 * @param maxRowsAffected the maximum number of rows that can be affected by this
	...
	/**
	 * Set the <i>exact</i> number of rows that must be affected by this update. The
	 * default value is 0, which allows any number of rows to be affected.
	 * <p>
	 * This is an alternative to setting the <i>maximum</i> number of rows that may be
	 * affected.
	 * @param requiredRowsAffected the exact number of rows that must be affected by this
	 * update without this class's update method considering it an error
	 */
	...
	/**
	 * Check the given number of affected rows against the specified maximum number or
	 * required number.
	 * @param rowsAffected the number of affected rows
	 * @throws JdbcUpdateAffectedIncorrectNumberOfRowsException if the actually affected
	...
	/**
	 * Generic method to execute the update given parameters. All other update methods
	 * invoke this method.
	...
	/**
	 * Method to execute the update given arguments and retrieve the generated keys using
	 * a KeyHolder.
	...
	/**
	 * Generic method to execute the update given named parameters. All other update
	 * methods invoke this method.
	 * @param paramMap a Map of parameter name to parameter object, matching named
	 * parameters specified in the SQL statement
	 * @return the number of rows affected by the update
	 */
