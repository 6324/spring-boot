...
/**
 * Implementation of {@link SmartDataSource} that wraps a single JDBC Connection which is
 * not closed after use. Obviously, this is not multi-threading capable.
 *
 * <p>
 * Note that at shutdown, someone should close the underlying Connection via the
 * {@code close()} method. Client code will never call close on the Connection handle if
 * it is SmartDataSource-aware (e.g. uses {@code DataSourceUtils.releaseConnection}).
 *
 * <p>
 * If client code will call {@code close()} in the assumption of a pooled Connection, like
 * when using persistence tools, set "suppressClose" to "true". This will return a
 * close-suppressing proxy instead of the physical Connection.
 *
 * <p>
 * This is primarily intended for testing. For example, it enables easy testing outside an
 * application server, for code that expects to work on a DataSource. In contrast to
 * {@link DriverManagerDataSource}, it reuses the same Connection all the time, avoiding
...
public class SingleConnectionDataSource extends DriverManagerDataSource implements SmartDataSource, DisposableBean {
	...
	/**
	 * Create a new SingleConnectionDataSource with the given standard DriverManager
	 * parameters.
	 * @param url the JDBC URL to use for accessing the DriverManager
	 * @param username the JDBC username to use for accessing the DriverManager
	 * @param password the JDBC password to use for accessing the DriverManager
	 * @param suppressClose if the returned Connection should be a close-suppressing proxy
	 ...
	 * suppresses {@code close()} calls (to allow for normal {@code close()} usage in
	 * applications that expect a pooled Connection but do not know our SmartDataSource
	 * interface)
	 */
	...
	/**
	 * Set whether the returned Connection should be a close-suppressing proxy or the
	 * physical Connection.
	 */
	...
	/**
	 * Return whether the returned Connection will be a close-suppressing proxy or the
	 * physical Connection.
	 */
	...
	/**
	 * Specifying a custom username and password doesn't make sense with a single
	 * Connection. Returns the single Connection if given the same username and password;
	 * throws an SQLException else.
	 */
	@Override
	...
	/**
	 * Close the underlying Connection. The provider of this DataSource needs to care for
	 * proper shutdown.
	 * <p>
	 ...
	 * <p>
	 * The default implementation applies the auto-commit flag, if necessary. Can be
