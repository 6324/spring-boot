 ...
 * {@link SimpleJdbcCall}. This interface is not often used directly, but provides the
...
public interface SimpleJdbcCallOperations {
	...
	/**
	 * Specify the procedure name to be used - this implies that we will be calling a
	 * stored procedure.
	 ...
	 * <p>
	 * To provide consistency with the Oracle DatabaseMetaData, this is used to specify
	 * the package name if the procedure is declared as part of a package.
	...
	/**
	 * Specify one or more parameters if desired. These parameters will be supplemented
	 * with any parameter information retrieved from the database meta-data.
	 * <p>
	 * Note that only parameters declared as {@code SqlParameter} and
	 * {@code SqlInOutParameter} will be used to provide input values. This is different
	 * from the {@code StoredProcedure} class which - for backwards compatibility reasons
	...
	/**
	 * Used to specify when a ResultSet is returned by the stored procedure and you want
	 * it mapped by a {@link RowMapper}. The results will be returned using the parameter
	 * name specified. Multiple ResultSets must be declared in the correct order.
	 * <p>
	 ...
	 * @param parameterName the name of the returned results and/or the name of the ref
	 * cursor parameter
	 * @param rowMapper the RowMapper implementation that will map the data returned for
	 * each row
	 */
	 ...
	 * @param args optional array containing the in parameter values to be used in the
	 * call. Parameter values must be provided in the same order as the parameters are
	 ...
	 * specified return type.
	 * @param returnType the type of the value to return
	 * @param args the MapSqlParameterSource containing the parameter values to be used in
	 * the call
	 */
	<T> T executeFunction(Class<T> returnType, SqlParameterSource args);

	/**
	 * Execute the stored procedure and return the single out parameter as an Object of
	 * the specified return type. In the case where there are multiple out parameters, the
	 * first one is returned and additional out parameters are ignored.
	 * @param returnType the type of the value to return
	 * @param args optional array containing the in parameter values to be used in the
	 * call. Parameter values must be provided in the same order as the parameters are
	 * defined for the stored procedure.
	...
	/**
	 * Execute the stored procedure and return a map of output params, keyed by name as in
	 ...
	 * Execute the stored procedure and return a map of output params, keyed by name as in
	 * parameter declarations.
	 * @param args the SqlParameterSource containing the parameter values to be used in
	 * the call
	 * @return a Map of output params
	 */
