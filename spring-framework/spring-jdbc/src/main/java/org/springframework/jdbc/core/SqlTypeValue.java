...
/**
 * Interface to be implemented for setting values for more complex database-specific types
 * not supported by the standard {@code setObject} method. This is effectively an extended
 * variant of {@link org.springframework.jdbc.support.SqlValue}.
 *
 * <p>
 * Implementations perform the actual work of setting the actual values. They must
 * implement the callback method {@code setTypeValue} which can throw SQLExceptions that
 * will be caught and translated by the calling code. This callback method has access to
 * the underlying Connection via the given PreparedStatement object, if that should be
...
public interface SqlTypeValue {
	 ...
	 * {@code setTypeValue} if the original operation method does not specify an SQL type.
	 * @see java.sql.Types
	 ...
	 * @throws SQLException if an SQLException is encountered while setting parameter
	 * values
	 * @see java.sql.Types
	 * @see java.sql.PreparedStatement#setObject
	 */
