 ...
 *
 * <p>
 * Delegates to the {@code streamData} template method for streaming LOB content to some
 * OutputStream, typically using a LobHandler. Converts an IOException thrown during
 * streaming to a LobRetrievalFailureException.
 *
 * <p>
...
public abstract class AbstractLobStreamingResultSetExtractor<T> implements ResultSetExtractor<T> {
	...
	/**
	 * Delegates to handleNoRowFound, handleMultipleRowsFound and streamData, according to
	 * the ResultSet state. Converts an IOException thrown by streamData to a
	 ...
	 * <p>
	 * Typically used as inner class, with access to surrounding method arguments and to a
