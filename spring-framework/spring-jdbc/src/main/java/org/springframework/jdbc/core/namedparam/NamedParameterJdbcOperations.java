...
/**
 * Interface specifying a basic set of JDBC operations allowing the use of named
 * parameters rather than the traditional '?' placeholders.
 *
 * <p>
 * This is an alternative to the classic
 * {@link org.springframework.jdbc.core.JdbcOperations} interface, implemented by
 * {@link NamedParameterJdbcTemplate}. This interface is not often used directly, but
 * provides a useful option to enhance testability, as it can easily be mocked or stubbed.
 *
...
public interface NamedParameterJdbcOperations {
	...
	/**
	 * Expose the classic Spring JdbcTemplate to allow invocation of classic JDBC
	 * operations.
	 */
	 ...
	 * collection of domain objects.
	 * @param sql the SQL to execute
	 * @param paramMap map of parameters to bind to the query (leaving it to the
	...
	/**
	 * Query given SQL to create a prepared statement from SQL, reading the ResultSet with
	 * a ResultSetExtractor.
	 * <p>
	 * Note: In contrast to the JdbcOperations method with the same signature, this query
	 * variant always uses a PreparedStatement. It is effectively equivalent to a query
	 * call with an empty parameter Map.
	 * @param sql the SQL query to execute
	 * @param rse object that will extract results
	 * @return an arbitrary result object, as returned by the ResultSetExtractor
	 * @throws DataAccessException if the query fails
	...
	/**
	 * Query given SQL to create a prepared statement from SQL, reading the ResultSet on a
	 * per-row basis with a RowCallbackHandler.
	 * <p>
	 * Note: In contrast to the JdbcOperations method with the same signature, this query
	 * variant always uses a PreparedStatement. It is effectively equivalent to a query
	 * call with an empty parameter Map.
	...
	/**
	 * Query given SQL to create a prepared statement from SQL and a list of arguments to
	 * bind to the query, mapping each row to a Java object via a RowMapper.
	 * @param sql the SQL query to execute
	 * @param paramSource container of arguments to bind to the query
	...
	/**
	 * Query given SQL to create a prepared statement from SQL, mapping each row to a Java
	...
	/**
	 * Query given SQL to create a prepared statement from SQL and a list of arguments to
	 * bind to the query, mapping a single result row to a Java object via a RowMapper.
	 * @param sql the SQL query to execute
	 * @param paramSource container of arguments to bind to the query
	 * @param rowMapper object that will map one object per row
	...
	/**
	 * Query given SQL to create a prepared statement from SQL and a list of arguments to
	 * bind to the query, expecting a result Map. The queryForMap() methods defined by
	 * this interface are appropriate when you don't have a domain model. Otherwise,
	 * consider using one of the queryForObject() methods.
	 * <p>
	 * The query is expected to be a single row query; the result row will be mapped to a
	 * Map (one entry for each column, using the column name as the key).
	 * @param sql the SQL query to execute
	...
	/**
	 * Issue an update via a prepared statement, binding the given arguments, returning
	 * generated keys.
	 * @param sql the SQL containing named parameters
	 * @param paramSource container of arguments and SQL types to bind to the query
	...
	/**
	 * Executes a batch using the supplied SQL statement with the batch of supplied
	 * arguments.
	 * @param sql the SQL statement to execute
	 * @param batchValues the array of Maps containing the batch of arguments for the
	 * query
