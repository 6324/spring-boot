 ...
 *
 * <p>
 * Useful for test or standalone environments outside of a Java EE container, either as a
 * DataSource bean in a corresponding ApplicationContext or in conjunction with a simple
 * JNDI environment. Pool-assuming {@code Connection.close()} calls will simply close the
 * Connection, so any DataSource-aware persistence code should work.
 *
 * <p>
 * <b>NOTE: Within special class loading environments such as OSGi, this class is
 * effectively superseded by {@link SimpleDriverDataSource} due to general class loading
 * issues with the JDBC DriverManager that be resolved through direct Driver usage (which
 * is exactly what SimpleDriverDataSource does).</b>
 *
 * <p>
 ...
 * seamless switching to and from a local DataSource bean like this class. For tests, you
 * can then either set up a mock JNDI environment through Spring's
 * {@link org.springframework.mock.jndi.SimpleNamingContextBuilder}, or switch the bean
 * definition to a local DataSource (which is simpler and thus recommended).
 *
 * <p>
 * This {@code DriverManagerDataSource} class was originally designed alongside
...
public class DriverManagerDataSource extends AbstractDriverBasedDataSource {
	...
	/**
	 * Create a new DriverManagerDataSource with the given JDBC URL, not specifying a
	 * username or password for JDBC access.
	...
	/**
	 * Create a new DriverManagerDataSource with the given standard DriverManager
	 * parameters.
	 * @param url the JDBC URL to use for accessing the DriverManager
	...
	/**
	 * Set the JDBC driver class name. This driver will get initialized on startup,
	 * registering itself with the JDK's DriverManager.
	 * <p>
	 * <b>NOTE: DriverManagerDataSource is primarily intended for accessing
	 ...
	 * initializing the JDBC driver yourself before instantiating this DataSource. The
	 * "driverClassName" property is mainly preserved for backwards compatibility, as well
	 * as for migrating between Commons DBCP and this DataSource.
