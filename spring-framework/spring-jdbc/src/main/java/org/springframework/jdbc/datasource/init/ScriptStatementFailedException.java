...
/**
 * Thrown by {@link ScriptUtils} if a statement in an SQL script failed when executing it
...
public class ScriptStatementFailedException extends ScriptException {
	 ...
	 * @param stmtNumber the statement number in the SQL script (i.e., the n<sup>th</sup>
	 * statement present in the resource)
	...
	/**
	 * Build an error message for an SQL script execution failure, based on the supplied
	 * arguments.
	 * @param stmt the actual SQL statement that failed
	 * @param stmtNumber the statement number in the SQL script (i.e., the n<sup>th</sup>
	 * statement present in the resource)
	 * @param encodedResource the resource from which the SQL statement was read
	 * @return an error message suitable for an exception's <em>detail message</em> or
	 * logging
	 * @since 4.2
	 */
