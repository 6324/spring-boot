 ...
 * mapRow(ResultSet, int) method to map each row of the JDBC ResultSet into an object.
 *
 * <p>
 * Such manual mapping is usually preferable to "automatic" mapping using reflection,
 * which can become complex in non-trivial cases. For example, the present class allows
 * different objects to be used for different rows (for example, if a subclass is
 * indicated). It allows computed fields to be set. And there's no need for ResultSet
 * columns to have the same names as bean properties. The Pareto Principle in action:
 * going the extra mile to automate the extraction process makes the framework much more
 * complex and delivers little real benefit.
 *
 * <p>
...
public abstract class MappingSqlQueryWithParameters<T> extends SqlQuery<T> {
	...
	/**
	 * Subclasses must implement this method to convert each row of the ResultSet into an
	 * object of the result type.
	 * @param rs the ResultSet we're working through
	 * @param rowNum row number (from 0) we're up to
	 * @param parameters to the query (passed to the execute() method). Subclasses are
	 * rarely interested in these. It can be {@code null} if there are no parameters.
	 * @param context passed to the execute() method. It can be {@code null} if no
	 * contextual information is need.
	 * @return an object of the result type
	 * @throws SQLException if there's an error extracting data. Subclasses can simply not
	...
	/**
	 * Implementation of RowMapper that calls the enclosing class's {@code mapRow} method
	 * for each row.
	 */
