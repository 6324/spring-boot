 ...
 *
 * <p>
 * Also includes tests proving that using {@link Lazy} and {@link Primary} annotations in
...
public class BeanAnnotationAttributePropagationTests {
	...
	public void autowireMetadataIsPropagated() {
		...
		assertThat(beanDef(Config.class).getAutowireMode()).as("autowire mode was not propagated")
	...
	public void autowireCandidateMetadataIsPropagated() {
		...
		assertThat(beanDef(Config.class).isAutowireCandidate()).as("autowire candidate flag was not propagated")
				.isFalse();
	}
	...
	public void initMethodMetadataIsPropagated() {
		...
		assertThat(beanDef(Config.class).getInitMethodName()).as("init method name was not propagated")
				.isEqualTo("start");
	}
	...
	public void destroyMethodMetadataIsPropagated() {
		...
		assertThat(beanDef(Config.class).getDestroyMethodName()).as("destroy method name was not propagated")
				.isEqualTo("destroy");
	}
	...
	public void dependsOnMetadataIsPropagated() {
		...
		assertThat(beanDef(Config.class).getDependsOn()).as("dependsOn metadata was not propagated")
	...
	public void defaultLazyConfigurationPropagatesToIndividualBeans() {
		...
		assertThat(beanDef(Config.class).isLazyInit())
				.as("@Bean methods declared in a @Lazy @Configuration should be lazily instantiated").isTrue();
	}
	...
	public void eagerConfigurationProducesEagerBeanDefinitions() {
		...
		assertThat(beanDef(Config.class).isLazyInit())
				.as("@Lazy(false) @Configuration should produce eager bean definitions").isFalse();
	}
