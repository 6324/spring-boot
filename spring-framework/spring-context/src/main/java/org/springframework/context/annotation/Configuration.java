...
/**
 * Indicates that a class declares one or more {@link Bean @Bean} methods and may be
 ...
 *
 * <p>
 * See the {@link AnnotationConfigApplicationContext} javadocs for further details, and
 * see
 * {@link org.springframework.web.context.support.AnnotationConfigWebApplicationContext
 ...
 * enable {@link ConfigurationClassPostProcessor} and other annotation-related post
 * processors that facilitate handling {@code @Configuration} classes.
 *
 ...
 * advantage of {@link Autowired @Autowired}/{@link javax.inject.Inject @Inject} like any
 * regular {@code @Component}. In particular, if a single constructor is present
 ...
 *
 * <p>
 * {@code @Configuration} classes may not only be bootstrapped using component scanning,
 * but may also themselves <em>configure</em> component scanning using the
 * {@link ComponentScan @ComponentScan} annotation:
 *
 * <pre class="code">
 * &#064;Configuration
 ...
 * {@link org.springframework.core.env.Environment} into a {@code @Configuration} class
 * &mdash; for example, using the {@code @Autowired} annotation:
 *
 * <pre class="code">
 * &#064;Configuration
 * public class AppConfig {
 ...
 *
 * <p>
 * See the {@link org.springframework.core.env.Environment Environment} and
 * {@link PropertySource @PropertySource} javadocs for further details.
 *
 * <h3>Using the {@code @Value} annotation</h3>
 *
 * <p>
 * Externalized values may be injected into {@code @Configuration} classes using the
 ...
 * {@code @Configuration} class via a dedicated {@code static} {@code @Bean} method (see
 * "a note on BeanFactoryPostProcessor-returning {@code @Bean} methods" of
 ...
 * {@code @Configuration} classes with Spring XML using {@code @ImportResource}; see the
 * {@link Value @Value} javadocs; and see the {@link Bean @Bean} javadocs for details on
 * working with {@code BeanFactoryPostProcessor} types such as
 ...
 * annotation, similar to the way that {@code <import>} works in Spring XML. Because
 ...
 *
 * <p>
 * {@code @Configuration} classes may be marked with the {@link Profile @Profile}
 * annotation to indicate they should be processed only if a given profile or profiles are
 * <em>active</em>:
 *
 * <pre class="code">
 * &#064;Profile("development")
 ...
 *
 * <p>
 * Alternatively, you may also declare profile conditions at the {@code @Bean} method
 * level &mdash; for example, for alternative bean variants within the same configuration
 * class:
 *
 * <pre class="code">
 * &#064;Configuration
 ...
 * {@code <bean>} definitions within Spring XML files. It is also possible to import
 * Spring XML configuration files into {@code @Configuration} classes using the
 * {@link ImportResource @ImportResource} annotation. Bean definitions imported from XML
 * can be injected &mdash; for example, using the {@code @Inject} annotation:
 *
 * <pre class="code">
 * &#064;Configuration
 ...
 *
 * <p>
 * Note also that nested {@code @Configuration} classes can be used to good effect with
 * the {@code @Profile} annotation to provide two options of the same bean to the
 * enclosing {@code @Configuration} class.
 *
 * <h2>Configuring lazy initialization</h2>
 *
 * <p>
 * By default, {@code @Bean} methods will be <em>eagerly instantiated</em> at container
 ...
 * <p>
 * See the <a href=
 * "https://docs.spring.io/spring/docs/current/spring-framework-reference/testing.html#testcontext-framework">TestContext
 ...
 * unless the {@link #proxyBeanMethods() proxyBeanMethods} flag is set to {@code false} in
 * which case no runtime-generated subclass is necessary.
 ...
 * <li>{@code @Bean} methods may not in turn create further configuration classes (any
 * such instances will be treated as regular beans, with their configuration annotations
...
public @interface Configuration {
	 ...
	 * {@code @Configuration} class. If left unspecified (the common case), a bean name
	 * will be automatically generated.
	 * <p>
	 * The custom name applies only if the {@code @Configuration} class is picked up via
	 * component scanning or supplied directly to an
	 * {@link AnnotationConfigApplicationContext}. If the {@code @Configuration} class is
	 * registered as a traditional XML bean definition, the name/id of the bean element
	 ...
	 * direct {@code @Bean} method calls in user code. This feature requires method
	 * interception, implemented through a runtime-generated CGLIB subclass which comes
	 * with limitations such as the configuration class and its methods not being allowed
	 * to declare {@code final}.
	 * <p>
	 * The default is {@code true}, allowing for 'inter-bean references' via direct method
	 * calls within the configuration class as well as for external calls to this
	 * configuration's {@code @Bean} methods, e.g. from another configuration class. If
	 * this is not needed since each of this particular configuration's {@code @Bean}
