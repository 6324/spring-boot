 ...
 *
 * <p>
 * Note that the {@code <context:component-scan>} element has an {@code annotation-config}
 * attribute; however, this annotation does not. This is because in almost all cases when
 * using {@code @ComponentScan}, default annotation config processing (e.g. processing
 * {@code @Autowired} and friends) is assumed. Furthermore, when using
 * {@link AnnotationConfigApplicationContext}, annotation config processors are always
 * registered, meaning that any attempt to disable them at the {@code @ComponentScan}
...
public @interface ComponentScan {
	 ...
	 * the scanner used to process this {@code @ComponentScan} annotation should use its
	 * inherited bean name generator, e.g. the default {@link AnnotationBeanNameGenerator}
	...
	/**
	 * The {@link ScopeMetadataResolver} to be used for resolving the scope of detected
	 * components.
	 */
