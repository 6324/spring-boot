...
/**
 * Activates a Spring {@link LoadTimeWeaver} for this application context, available as a
 * bean with the name "loadTimeWeaver", similar to the {@code <context:load-time-weaver>}
 * element in Spring XML.
 *
 * <p>
 * To be used on @{@link org.springframework.context.annotation.Configuration
 * Configuration} classes; the simplest possible example of which follows:
 *
 * <pre class="code">
 * &#064;Configuration
 ...
 * </pre>
 *
 * <h2>The {@code LoadTimeWeaverAware} interface</h2> Any bean that implements the
 * {@link org.springframework.context.weaving.LoadTimeWeaverAware LoadTimeWeaverAware}
 * interface will then receive the {@code LoadTimeWeaver} reference automatically; for
 * example, Spring's JPA bootstrap support.
 *
 * <h2>Customizing the {@code LoadTimeWeaver}</h2> The default weaver is determined
 * automatically: see {@link DefaultContextLoadTimeWeaver}.
 *
 * <p>
 ...
 *
 * <h2>Enabling AspectJ-based weaving</h2> AspectJ load-time weaving may be enabled with
 * the {@link #aspectjWeaving()} attribute, which will cause the
 * {@linkplain org.aspectj.weaver.loadtime.ClassPreProcessorAgentAdapter AspectJ class
 * transformer} to be registered through {@link LoadTimeWeaver#addTransformer}. AspectJ
 * weaving will be activated by default if a "META-INF/aop.xml" resource is present on the
 * classpath. Example:
 *
 * <pre class="code">
 * &#064;Configuration
 ...
 *
 * <p>
 * The two examples are equivalent with one significant exception: in the XML case, the
 * functionality of {@code <context:spring-configured>} is implicitly enabled when
...
public @interface EnableLoadTimeWeaving {
	...
	enum AspectJWeaving {
		...
		/**
		 * Switches on AspectJ load-time weaving if a "META-INF/aop.xml" resource is
		 * present in the classpath. If there is no such resource, then AspectJ load-time
