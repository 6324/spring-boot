 ...
 * {@code testbean.name=myTestBean}, the following {@code @Configuration} class uses
 * {@code @PropertySource} to contribute {@code app.properties} to the
 ...
 *
 * <h3>Resolving <code>${...}</code> placeholders in {@code <bean>} and {@code @Value}
 * annotations</h3>
 *
 * <p>
 ...
 * {@code <context:property-placeholder>} in XML. When using {@code @Configuration}
 * classes this can be achieved by explicitly registering a
 * {@code PropertySourcesPlaceholderConfigurer} via a {@code static} {@code @Bean} method.
 * Note, however, that explicit registration of a
 * {@code PropertySourcesPlaceholderConfigurer} via a {@code static} {@code @Bean} method
 * is typically only required if you need to customize configuration such as the
 ...
 *
 * <h3>Resolving ${...} placeholders within {@code @PropertySource} resource
 * locations</h3>
 *
 * <p>
 ...
 * resolved to the corresponding value. If not, then "default/path" will be used as a
 * default. Expressing a default value (delimited by colon ":") is optional. If no default
 * is specified and a property cannot be resolved, an {@code
 * IllegalArgumentException} will be thrown.
 *
 * <h3>A note on property overriding with @PropertySource</h3>
 *
 * <p>
 * In cases where a given property key exists in more than one {@code .properties} file,
 * the last {@code @PropertySource} annotation processed will 'win' and override.
 *
 * <p>
 * For example, given two properties files {@code a.properties} and {@code b.properties},
 * consider the following two configuration classes that reference them with
 * {@code @PropertySource} annotations:
 *
 * <pre class="code">
 * &#064;Configuration
 ...
 *
 * <p>
 * The override ordering depends on the order in which these classes are registered with
 ...
 * source ordering when using {@code @PropertySource} annotations. For example, if the
 * {@code @Configuration} classes above were registered via component-scanning, the
 ...
 *
 * <p>
 * <b>NOTE: This annotation is repeatable according to Java 8 conventions.</b> However,
 * all such {@code @PropertySource} annotations need to be declared at the same level:
 * either directly on the configuration class or as meta-annotations within the same
 * custom annotation. Mixing of direct annotations and meta-annotations is not recommended
 * since direct annotations will effectively override meta-annotations.
 *
 * @author Chris Beams
...
public @interface PropertySource {
	...
	/**
	 * Indicate the name of this property source. If omitted, the {@link #factory()} will
	 * generate a name based on the underlying resource (in the case of
	 * {@link org.springframework.core.io.support.DefaultPropertySourceFactory}: derived
	 * from the resource description through a corresponding name-less
	 ...
	 * <p>
	 * Both traditional and XML-based properties file formats are supported &mdash; for
	 * example, {@code "classpath:/com/myco/app.properties"} or
	 * {@code "file:/path/to/file.xml"}.
	 * <p>
	 ...
	 * ignored.
	 * <p>
	 * {@code true} is appropriate if the properties file is completely optional. Default
	 * is {@code false}.
	 * @since 4.0
	 */
