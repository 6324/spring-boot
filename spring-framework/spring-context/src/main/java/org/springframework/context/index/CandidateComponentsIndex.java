 ...
 * {@code META-INF/spring.components}.
 *
 * <p>
 * An arbitrary number of stereotypes can be registered (and queried) on the index: a
 * typical example is the fully qualified name of an annotation that flags the class for a
 * certain use case. The following call returns all the {@code @Component}
 ...
 *
 * <p>
 * The {@code type} is usually the fully qualified name of a class, though this is not a
 * rule. Similarly, the {@code stereotype} is usually the fully qualified name of a target
 * type but it can be any marker really.
 *
...
public class CandidateComponentsIndex {
	 ...
	 * @return the candidate types associated with the specified {@code stereotype} or an
	 * empty set if none has been found for the specified {@code basePackage}
	 */
	public Set<String> getCandidateTypes(String basePackage, String stereotype) {
		List<Entry> candidates = this.index.get(stereotype);
		if (candidates != null) {
			return candidates.parallelStream().filter(t -> t.match(basePackage)).map(t -> t.type)
