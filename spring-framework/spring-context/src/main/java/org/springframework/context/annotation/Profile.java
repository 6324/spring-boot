 ...
 *
 * <p>
 * A <em>profile</em> is a named logical grouping that may be activated programmatically
 * via {@link ConfigurableEnvironment#setActiveProfiles} or declaratively by setting the
 * {@link AbstractEnvironment#ACTIVE_PROFILES_PROPERTY_NAME spring.profiles.active}
 * property as a JVM system property, as an environment variable, or as a Servlet context
 * parameter in {@code web.xml} for web applications. Profiles may also be activated
 * declaratively in integration tests via the {@code @ActiveProfiles} annotation.
 *
 * <p>
 ...
 * with {@code @Profile({"p1", "p2"})}, that class will not be registered or processed
 * unless at least profile 'p1' or 'p2' has been activated.
 *
 * <p>
 ...
 * given {@code @Profile({"p1", "!p2"})}, registration will occur if profile 'p1' is
 * active or if profile 'p2' is <em>not</em> active.
 *
 * <p>
 * If the {@code @Profile} annotation is omitted, registration will occur regardless of
 * which (if any) profiles are active.
 *
 * <p>
 * <b>NOTE:</b> With {@code @Profile} on {@code @Bean} methods, a special scenario may
 ...
 * particular argument signature over another; resolution between all factory methods for
 * the same bean follows Spring's constructor resolution algorithm at creation time.
 * <b>Use distinct Java method names pointing to the same {@link Bean#name bean name} if
 * you'd like to define alternative beans with different profile conditions</b>; see
 * {@code ProfileDatabaseConfig} in {@link Configuration @Configuration}'s javadoc.
 *
 * <p>
 * When defining Spring beans via XML, the {@code "profile"} attribute of the
 * {@code <beans>} element may be used. See the documentation in the {@code spring-beans}
