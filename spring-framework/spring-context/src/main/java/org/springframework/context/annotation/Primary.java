 ...
 * to autowire a single-valued dependency. If exactly one 'primary' bean exists among the
 * candidates, it will be the autowired value.
 *
 * <p>
 * This annotation is semantically equivalent to the {@code <bean>} element's
 * {@code primary} attribute in Spring XML.
 *
 * <p>
 * May be used on any class directly or indirectly annotated with {@code @Component} or on
 * methods annotated with @{@link Bean}.
 *
 * <h2>Example</h2> <pre class="code">
 * &#064;Component
 ...
 *
 * <p>
 * Because {@code HibernateFooRepository} is marked with {@code @Primary}, it will be
 * injected preferentially over the jdbc-based variant assuming both are present as beans
 * within the same Spring application context, which is often the case when
