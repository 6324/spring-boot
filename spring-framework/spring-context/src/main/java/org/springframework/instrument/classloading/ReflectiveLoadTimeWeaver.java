 ...
 * with well-known transformation hooks. The underlying ClassLoader is expected to support
 * the following weaving methods (as defined in the {@link LoadTimeWeaver} interface):
 * <ul>
 ...
 * <li>{@code public ClassLoader getThrowawayClassLoader()}: for obtaining a throwaway
 * class loader for this ClassLoader (optional; ReflectiveLoadTimeWeaver will fall back to
 * a SimpleThrowawayClassLoader if that method isn't available)
 * </ul>
 *
 * <p>
 * Please note that the above methods <i>must</i> reside in a class that is publicly
 * accessible, although the class itself does not have to be visible to the application's
 * class loader.
 *
 * <p>
 * The reflective nature of this LoadTimeWeaver is particularly useful when the underlying
 * ClassLoader implementation is loaded in a different class loader itself (such as the
 * application server's class loader which is not visible to the web application). There
 * is no direct API dependency between this LoadTimeWeaver adapter and the underlying
 * ClassLoader, just a 'loose' method contract.
 *
 * <p>
...
public class ReflectiveLoadTimeWeaver implements LoadTimeWeaver {
	 ...
	 * <i>which needs to support the required weaving methods</i>.
	 */
	 ...
	 * @param classLoader the {@code ClassLoader} to delegate to for weaving (<i>must</i>
	 * support the required weaving methods).
	 * @throws IllegalStateException if the supplied {@code ClassLoader} does not support
	 * the required weaving methods
	 */
