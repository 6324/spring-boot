...
/**
 * {@link org.aopalliance.intercept.MethodInterceptor} for accessing RMI services from
 * JNDI. Typically used for RMI-IIOP but can also be used for EJB home objects (for
 * example, a Stateful Session Bean home). In contrast to a plain JNDI lookup, this
 * accessor also performs narrowing through PortableRemoteObject.
 *
 * <p>
 ...
 *
 * <p>
 * The JNDI environment can be specified as "jndiEnvironment" property, or be configured
 * in a {@code jndi.properties} file or as system properties. For example:
 *
...
public class JndiRmiClientInterceptor extends JndiObjectLocator implements MethodInterceptor, InitializingBean {
	...
	/**
	 * Set the interface of the service to access. The interface must be suitable for the
	 * particular service and remoting tool.
	 * <p>
	 * Typically required to be able to create a suitable service proxy, but can also be
	 * optional if the lookup returns a typed stub.
	 */
	...
	/**
	 * Set whether to expose the JNDI environment context for all access to the target RMI
	 * stub, i.e. for all method invocations on the exposed object reference.
	 * <p>
	 ...
	 * authorization context) for each RMI invocation, as needed by WebLogic for RMI stubs
		...
		if (this.lookupStubOnStartup) {
			...
			if (logger.isDebugEnabled()) {
				if (remoteObj instanceof RmiInvocationHandler) {
					...
					logger.debug(
							"Using service interface [" + getServiceInterface().getName() + "] for JNDI RMI object ["
									+ getJndiName() + "] - " + (!isImpl ? "not " : "") + "directly implemented");
				}
			}
			if (this.cacheStub) {
	 ...
	 * <p>
	 * The default implementation retrieves the service from the JNDI environment. This
	...
	/**
	 * Fetches an RMI stub and delegates to {@link #doInvoke}. If configured to refresh on
	...
	/**
	 * Convert the given RMI RemoteException that happened during remote access to
