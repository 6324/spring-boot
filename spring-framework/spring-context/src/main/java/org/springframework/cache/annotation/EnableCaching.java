...
/**
 * Enables Spring's annotation-driven cache management capability, similar to the support
 * found in Spring's {@code <cache:*>} XML namespace. To be used together
 ...
 *
 * <p>
 * <strong>A bean of type {@link org.springframework.cache.CacheManager CacheManager} must
 * be registered</strong>, as there is no reasonable default that the framework can use as
 * a convention. And whereas the {@code <cache:annotation-driven>} element assumes a bean
 * <em>named</em> "cacheManager", {@code @EnableCaching} searches for a cache manager bean
 * <em>by type</em>. Therefore, naming of the cache manager bean method is not
 * significant.
 *
 * <p>
 * For those that wish to establish a more direct relationship between
 * {@code @EnableCaching} and the exact cache manager bean to be used, the
 * {@link CachingConfigurer} callback interface may be implemented. Notice the
 * {@code @Override}-annotated methods below:
 *
 * <pre class="code">
 * &#064;Configuration
 * &#064;EnableCaching
 ...
 * {@link org.springframework.cache.interceptor.KeyGenerator KeyGenerator} SPI. Normally,
 * {@code @EnableCaching} will configure Spring's
 * {@link org.springframework.cache.interceptor.SimpleKeyGenerator SimpleKeyGenerator} for
 * this purpose, but when implementing {@code CachingConfigurer}, a key generator must be
 * provided explicitly. Return {@code null} or {@code new SimpleKeyGenerator()} from this
 * method if no customization is necessary.
 *
 * <p>
 * {@link CachingConfigurer} offers additional customization options: it is recommended to
 * extend from {@link org.springframework.cache.annotation.CachingConfigurerSupport
...
public @interface EnableCaching {
	 ...
	 * cannot get intercepted that way; a caching annotation on such a method within a
	 * local call will be ignored since Spring's interceptor does not even kick in for
	 * such a runtime scenario. For a more advanced mode of interception, consider
	 * switching this to {@link AdviceMode#ASPECTJ}.
	 */
	AdviceMode mode() default AdviceMode.PROXY;

	/**
	 * Indicate the ordering of the execution of the caching advisor when multiple advices
	 * are applied at a specific joinpoint.
	 * <p>
