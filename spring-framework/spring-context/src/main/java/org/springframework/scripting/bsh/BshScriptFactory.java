...
/**
 * {@link org.springframework.scripting.ScriptFactory} implementation for a BeanShell
 * script.
 *
 * <p>
...
public class BshScriptFactory implements ScriptFactory, BeanClassLoaderAware {
	 ...
	 * <p>
	 * With this {@code BshScriptFactory} variant, the script needs to declare a full
	 * class or return an actual instance of the scripted object.
	...
	public Object getScriptedObject(ScriptSource scriptSource, @Nullable Class<?>... actualInterfaces)
		...
		try {
			synchronized (this.scriptClassMonitor) {
				...
				if (scriptSource.isModified() || requiresScriptEvaluation) {
					...
					Object result = BshScriptUtils.evaluateBshScript(scriptSource.getScriptAsString(), actualInterfaces,
		...
		if (clazz != null) {
			...
			try {
				return BshScriptUtils.createBshObject(scriptSource.getScriptAsString(), actualInterfaces,
						this.beanClassLoader);
			}
			catch (EvalError ex) {
				throw new ScriptCompilationException(scriptSource, ex);
