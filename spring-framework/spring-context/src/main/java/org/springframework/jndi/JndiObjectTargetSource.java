...
/**
 * AOP {@link org.springframework.aop.TargetSource} that provides configurable JNDI
 * lookups for {@code getTarget()} calls.
 *
 * <p>
 * Can be used as alternative to {@link JndiObjectFactoryBean}, to allow for relocating a
 * JNDI object lazily or for each operation (see "lookupOnStartup" and "cache"
 * properties). This is particularly useful during development, as it allows for hot
 * restarting of the JNDI server (for example, a remote JMS server).
 *
 * <p>
 * Example:
 *
 ...
 * &lt;/bean&gt;</pre>
 *
 * A {@code createQueueConnection} call on the "queueConnectionFactory" proxy will cause a
 * lazy JNDI lookup for "JmsQueueConnectionFactory" and a subsequent delegating call to
 * the retrieved QueueConnectionFactory's {@code createQueueConnection}.
 *
 * <p>
