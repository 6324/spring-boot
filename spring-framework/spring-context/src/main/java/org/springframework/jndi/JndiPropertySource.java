 ...
 * ApplicationContextInitializer} or
...
public class JndiPropertySource extends PropertySource<JndiLocatorDelegate> {
	...
	/**
	 * Create a new {@code JndiPropertySource} with the given name and a
	 * {@link JndiLocatorDelegate} configured to prefix any names with "java:comp/env/".
	 */
	...
	/**
	 * This implementation looks up and returns the value associated with the given name
	 * from the underlying {@link JndiLocatorDelegate}. If a {@link NamingException} is
	 * thrown during the call to {@link JndiLocatorDelegate#lookup(String)}, returns
	...
	public Object getProperty(String name) {
		if (getSource().isResourceRef() && name.indexOf(':') != -1) {
			// We're in resource-ref (prefixing with "java:comp/env") mode. Let's not
			// bother
			// with property names with a colon it since they're probably just containing
			// a
			// default value clause, very unlikely to match including the colon part even
			// in
			// a textual property source, and effectively never meant to match that way in
			// JNDI where a colon indicates a separator between JNDI scheme and actual
			// name.
			return null;
		}
