...
/**
 * {@link org.springframework.beans.factory.FactoryBean} that sets up a
 * {@link java.util.concurrent.ScheduledExecutorService} (by default: a
 * {@link java.util.concurrent.ScheduledThreadPoolExecutor}) and exposes it for bean
 * references.
 *
 * <p>
 ...
 * directly using constructor injection, or use a factory method definition that points to
 * the {@link java.util.concurrent.Executors} class. <b>This is strongly recommended in
 * particular for common {@code @Bean} methods in configuration classes, where this
 * {@code FactoryBean} variant would force you to return the {@code FactoryBean} type
 * instead of {@code ScheduledExecutorService}.</b>
 *
 * <p>
 * Note that {@link java.util.concurrent.ScheduledExecutorService} uses a {@link Runnable}
 * instance that is shared between repeated executions, in contrast to Quartz which
 * instantiates a new Job for each execution.
 *
 * <p>
 * <b>WARNING:</b> {@link Runnable Runnables} submitted via a native
 * {@link java.util.concurrent.ScheduledExecutorService} are removed from the execution
 * schedule once they throw an exception. If you would prefer to continue execution after
 * such an exception, switch this FactoryBean's
 * {@link #setContinueScheduledExecutionAfterException
...
public class ScheduledExecutorFactoryBean extends ExecutorConfigurationSupport
	...
	/**
	 * Specify whether to continue the execution of a scheduled task after it threw an
	 * exception.
	 * <p>
	 * Default is "false", matching the native behavior of a
	 * {@link java.util.concurrent.ScheduledExecutorService}. Switch this flag to "true"
	 * for exception-proof execution of each task, continuing scheduled execution as in
	 * the case of successful execution.
	...
	/**
	 * Specify whether this FactoryBean should expose an unconfigurable decorator for the
	 * created executor.
	 * <p>
	 * Default is "false", exposing the raw executor as bean reference. Switch this flag
	 * to "true" to strictly prevent clients from modifying the executor's configuration.
	...
	/**
	 * Register the specified {@link ScheduledExecutorTask ScheduledExecutorTasks} on the
	 * given {@link ScheduledExecutorService}.
	 ...
	 * {@link org.springframework.scheduling.support.DelegatingErrorHandlingRunnable} that
	 * will catch and log the Exception. If necessary, it will suppress the Exception
	 * according to the {@link #setContinueScheduledExecutionAfterException
	 * "continueScheduledExecutionAfterException"} flag.
