...
/**
 * Base class for setting up a {@link java.util.concurrent.ExecutorService} (typically a
 * {@link java.util.concurrent.ThreadPoolExecutor} or
 * {@link java.util.concurrent.ScheduledThreadPoolExecutor}). Defines common configuration
...
public abstract class ExecutorConfigurationSupport extends CustomizableThreadFactory
	...
	/**
	 * Set the ThreadFactory to use for the ExecutorService's thread pool. Default is the
	 * underlying ExecutorService's default thread factory.
	 * <p>
	 * In a Java EE 7 or other managed environment with JSR-236 support, consider
	 * specifying a JNDI-located ManagedThreadFactory: by default, to be found at
	 * "java:comp/DefaultManagedThreadFactory". Use the "jee:jndi-lookup" namespace
	 * element in XML or the programmatic
	 ...
	 * running tasks and executing all tasks in the queue.
	 * <p>
	 * Default is "false", shutting down immediately through interrupting ongoing tasks
	 ...
	 * completed. If you want this executor to block and wait for the termination of tasks
	 * before the rest of the container continues to shut down - e.g. in order to keep up
	 * other resources that your tasks may need -, set the
	 * {@link #setAwaitTerminationSeconds "awaitTerminationSeconds"} property instead of
	 * or in addition to this property.
	 ...
	 * <p>
	 * By default, this executor won't wait for the termination of tasks at all. It will
	 * either shut down immediately, interrupting ongoing tasks and clearing the remaining
	 * task queue - or, if the {@link #setWaitForTasksToCompleteOnShutdown
	 * "waitForTasksToCompleteOnShutdown"} flag has been set to {@code true}, it will
	 * continue to fully execute all ongoing tasks as well as all remaining tasks in the
	 ...
	 * this executor will wait for the given time (max) for the termination of tasks. As a
	 * rule of thumb, specify a significantly higher timeout here if you set
	 * "waitForTasksToCompleteOnShutdown" to {@code true} at the same time, since all
	 * remaining tasks in the queue will still get executed - in contrast to the default
	 * shutdown behavior where it's just about waiting for currently executing tasks that
	 * aren't reacting to thread interruption.
	...
	/**
	 * Create the target {@link java.util.concurrent.ExecutorService} instance. Called by
