...
/**
 * Interface that specifies a basic set of CCI operations on an EIS. Implemented by
 * CciTemplate. Not often used, but a useful option to enhance testability, as it can
 * easily be mocked or stubbed.
 *
 * <p>
 * Alternatively, the standard CCI infrastructure can be mocked. However, mocking this
 * interface constitutes significantly less work.
 *
...
public interface CciOperations {
	...
	/**
	 * Execute a request on an EIS with CCI, implemented as callback action working on a
	 * CCI Connection. This allows for implementing arbitrary data access operations,
	 * within Spring's managed CCI environment: that is, participating in Spring-managed
	 * transactions and converting JCA ResourceExceptions into Spring's
	...
	/**
	 * Execute a request on an EIS with CCI, implemented as callback action working on a
	 * CCI Interaction. This allows for implementing arbitrary data access operations on a
	 * single Interaction, within Spring's managed CCI environment: that is, participating
	 * in Spring-managed transactions and converting JCA ResourceExceptions into Spring's
	 * DataAccessException hierarchy.
	 * <p>
	 * The callback action can return a result object, for example a domain object or a
	 * collection of domain objects.
