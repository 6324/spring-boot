...
/**
 * An adapter for a target CCI {@link javax.resource.cci.ConnectionFactory}, applying the
 * given ConnectionSpec to every standard {@code getConnection()} call, that is,
 * implicitly invoking {@code getConnection(ConnectionSpec)} on the target. All other
 * methods simply delegate to the corresponding methods of the target ConnectionFactory.
 *
 * <p>
 * Can be used to proxy a target JNDI ConnectionFactory that does not have a
 * ConnectionSpec configured. Client code can work with the ConnectionFactory without
 * passing in a ConnectionSpec on every {@code getConnection()} call.
 *
 * <p>
 ...
 * {@code getConnection()} method of the target ConnectionFactory. This can be used to
 * keep a UserCredentialsConnectionFactoryAdapter bean definition just for the
 * <i>option</i> of implicitly passing in a ConnectionSpec if the particular target
...
public class ConnectionSpecConnectionFactoryAdapter extends DelegatingConnectionFactory {
	...
	/**
	 * Set a ConnectionSpec for this proxy and the current thread. The given
	 * ConnectionSpec will be applied to all subsequent {@code getConnection()} calls on
	...
	/**
	 * Remove any ConnectionSpec for this proxy from the current thread. A statically
	 * specified ConnectionSpec applies again afterwards.
	...
	/**
	 * Determine whether there is currently a thread-bound ConnectionSpec, using it if
	 * available, falling back to the statically specified "connectionSpec" property else.
	 * @see #doGetConnection
	 */
	@Override
	...
	/**
	 * This implementation delegates to the {@code getConnection(ConnectionSpec)} method
	 * of the target ConnectionFactory, passing in the specified user credentials. If the
