...
/**
 * Enables Spring's annotation-driven transaction management capability, similar to the
 * support found in Spring's {@code <tx:*>} XML namespace. To be used on
 * {@link org.springframework.context.annotation.Configuration @Configuration} classes to
 * configure traditional, imperative transaction management or reactive transaction
 * management.
 *
 * <p>
 * The following example demonstrates imperative transaction management using a
 ...
 * TransactionManager} bean: In the {@code @Bean} case, the name is <em>"txManager"</em>
 ...
 * lookup for any {@code TransactionManager} bean in the container. Thus the name can be
 * "txManager", "transactionManager", or "tm": it simply does not matter.
 *
 * <p>
 ...
 * necessary in order to distinguish between two {@code TransactionManager} beans present
 * in the same container. As the name suggests, the
 * {@code annotationDrivenTransactionManager()} will be the one used for processing
 * {@code @Transactional} methods. See {@link TransactionManagementConfigurer} Javadoc for
 * further details.
 *
 * <p>
...
public @interface EnableTransactionManagement {
	 ...
	 * {@code @Async} annotation will be upgraded to subclass proxying at the same time.
	 * This approach has no negative impact in practice unless one is explicitly expecting
	 * one type of proxy vs another, e.g. in tests.
	 */
	 ...
	 * cannot get intercepted that way; an {@link Transactional} annotation on such a
	 * method within a local call will be ignored since Spring's interceptor does not even
	 * kick in for such a runtime scenario. For a more advanced mode of interception,
	 * consider switching this to {@link AdviceMode#ASPECTJ}.
	 */
	AdviceMode mode() default AdviceMode.PROXY;

	/**
	 * Indicate the ordering of the execution of the transaction advisor when multiple
