...
public class ExtendedBeanInfoTests {
	...
	@Test
	public void standardReadMethodInSuperAndSubclassesAndGenericBuilderStyleNonStandardWriteMethodInSuperAndSubclasses()
			throws Exception {
	...
	/**
	 * {@link ExtendedBeanInfo} should behave exactly like {@link BeanInfo} in strange
	 * edge cases.
	 */
	@Test
	...
	public void indexedReadAndNonStandardIndexedWrite() throws IntrospectionException {
		...
		assertThat(hasIndexedReadMethodForProperty(bi, "foos")).isTrue();
		// interesting! standard Inspector picks up non-void return types on indexed write
		// methods by default
		assertThat(hasIndexedWriteMethodForProperty(bi, "foos")).isFalse();
	...
	public void indexedReadAndNonStandardWriteAndNonStandardIndexedWrite() throws IntrospectionException {
		...
		assertThat(hasIndexedReadMethodForProperty(bi, "foos")).isTrue();
		assertThat(hasWriteMethodForProperty(bi, "foos")).isFalse();
		// again as above, standard Inspector picks up non-void return types on indexed
		// write methods by default
		assertThat(hasIndexedWriteMethodForProperty(bi, "foos")).isFalse();
	 ...
	 * methods" intermittently (approximately one out of every four times) under JDK 7 due
	 * to non-deterministic results from {@link Class#getDeclaredMethods()}. See
	...
	/**
	 * Ensures that an empty string is not passed into a PropertyDescriptor constructor.
	 * This could occur when handling ArrayList.set(int,Object)
	 */
	@Test
	 ...
	 * not actually intended to serve as an indexed write method; it just appears that
	 * way.
	 */
	@Test
	...
	/**
	 * {@link BeanInfo#getPropertyDescriptors()} returns alphanumerically sorted. Test
	 * that {@link ExtendedBeanInfo#getPropertyDescriptors()} does the same.
	 */
	@Test
	...
	public void propertyDescriptorComparator() throws IntrospectionException {
		...
		assertThat(c.compare(new PropertyDescriptor("a", null, null), new PropertyDescriptor("a", null, null)))
				.isEqualTo(0);
		assertThat(c.compare(new PropertyDescriptor("abc", null, null), new PropertyDescriptor("abc", null, null)))
				.isEqualTo(0);
		assertThat(c.compare(new PropertyDescriptor("a", null, null), new PropertyDescriptor("b", null, null)))
				.isLessThan(0);
		assertThat(c.compare(new PropertyDescriptor("b", null, null), new PropertyDescriptor("a", null, null)))
				.isGreaterThan(0);
		assertThat(c.compare(new PropertyDescriptor("abc", null, null), new PropertyDescriptor("abd", null, null)))
				.isLessThan(0);
		assertThat(c.compare(new PropertyDescriptor("xyz", null, null), new PropertyDescriptor("123", null, null)))
				.isGreaterThan(0);
		assertThat(c.compare(new PropertyDescriptor("a", null, null), new PropertyDescriptor("abc", null, null)))
				.isLessThan(0);
		assertThat(c.compare(new PropertyDescriptor("abc", null, null), new PropertyDescriptor("a", null, null)))
				.isGreaterThan(0);
		assertThat(c.compare(new PropertyDescriptor("abc", null, null), new PropertyDescriptor("b", null, null)))
				.isLessThan(0);

		assertThat(c.compare(new PropertyDescriptor(" ", null, null), new PropertyDescriptor("a", null, null)))
				.isLessThan(0);
		assertThat(c.compare(new PropertyDescriptor("1", null, null), new PropertyDescriptor("a", null, null)))
				.isLessThan(0);
		assertThat(c.compare(new PropertyDescriptor("a", null, null), new PropertyDescriptor("A", null, null)))
				.isGreaterThan(0);
	}
	...
	public void cornerSpr8949() throws IntrospectionException {
		...
		// methods, which in turn violates expectations in {@link ExtendedBeanInfo}
		// regarding
		// method equality. Spring's {@link ClassUtils#getMostSpecificMethod(Method,
		// Class)}
