 ...
 * nested (directly under a {@code <bean>}) tags.
 *
 * <p>
 * Decoration may also occur based on custom attributes applied to the {@code <bean>} tag.
 ...
 *
 * <p>
 * {@link BeanDefinitionDecorator BeanDefinitionDecorators} should be aware that they may
 * be part of a chain. In particular, a {@link BeanDefinitionDecorator} should be aware
 * that a previous {@link BeanDefinitionDecorator} may have replaced the original
 * {@link org.springframework.beans.factory.config.BeanDefinition} with a
 ...
 * which handles the chaining ensuring that only one proxy is created and that it contains
 * all interceptors from the chain.
 *
 * <p>
 * The parser locates a {@link BeanDefinitionDecorator} from the {@link NamespaceHandler}
