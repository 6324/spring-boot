 ...
 * values or no values at all for such bean properties. If an overriding properties file
 * does not have an entry for a certain bean property, the default context definition is
 * used.
 *
 * <p>
 * Note that the context definition <i>is not</i> aware of being overridden; so this is
 * not immediately obvious when looking at the XML definition file. Furthermore, note that
 * specified override values are always <i>literal</i> values; they are not translated
 * into bean references. This also applies when the original value in the XML bean
 * definition specifies a bean reference.
 *
 * <p>
 * In case of multiple PropertyOverrideConfigurers that define different values for the
 * same bean property, the <i>last</i> one will win (due to the overriding mechanism).
 *
 * <p>
 * Property values can be converted after reading them in, through overriding the
 * {@code convertPropertyValue} method. For example, encrypted values can be detected and
...
public class PropertyOverrideConfigurer extends PropertyResourceConfigurer {
	...
	/**
	 * Were there overrides for this bean? Only valid after processing has occurred at
	 * least once.
