 ...
 *
 * <p>
 * This is the basic client view of a bean container; further interfaces such as
 ...
 * application context)
 * <li>ApplicationEventPublisherAware's {@code setApplicationEventPublisher} (only
 ...
 * application context)
 * <li>ApplicationContextAware's {@code setApplicationContext} (only applicable when
 * running in an application context)
 * <li>ServletContextAware's {@code setServletContext} (only applicable when running in a
...
public interface BeanFactory {
	...
	/**
	 * Used to dereference a {@link FactoryBean} instance and distinguish it from beans
	 * <i>created</i> by the FactoryBean. For example, if the bean named
	 * {@code myJndiObject} is a FactoryBean, getting {@code &myJndiObject} will return
	 ...
	 * Singleton or Prototype design pattern. Callers may retain references to returned
	 * objects in the case of Singleton beans.
	 * <p>
	 ...
	 * by throwing a BeanNotOfRequiredTypeException if the bean is not of the required
	 * type. This means that ClassCastException can't be thrown on casting the result
	 * correctly, as can happen with {@link #getBean(String)}.
	 * <p>
	 * Translates aliases back to the corresponding canonical bean name.
	 * <p>
	 * Will ask the parent factory if the bean cannot be found in this factory instance.
	 ...
	 * (only applied when creating a new instance as opposed to retrieving an existing
	 * one)
	 * @return an instance of the bean
	 * @throws NoSuchBeanDefinitionException if there is no such bean definition
	 * @throws BeanDefinitionStoreException if arguments have been given but the affected
	 * bean isn't a prototype
	 * @throws BeansException if the bean could not be created
	 * @since 2.5
	 */
	 ...
	 * @throws NoUniqueBeanDefinitionException if more than one bean of the given type was
	 * found
	 * @throws BeansException if the bean could not be created
	 * @since 3.0
	...
	/**
	 * Return a provider for the specified bean, allowing for lazy on-demand retrieval of
	 ...
	 * canonical bean name.
	 * <p>
	 * If this factory is hierarchical, will ask any parent factory if the bean cannot be
	 * found in this factory instance.
	 * <p>
	 * If a bean definition or singleton instance matching the given name is found, this
	 * method will return {@code true} whether the named bean definition is concrete or
	 * abstract, lazy or eager, in scope or not. Therefore, note that a {@code true}
	...
	/**
	 * Is this bean a shared singleton? That is, will {@link #getBean} always return the
	 * same instance?
	 * <p>
	 * Note: This method returning {@code false} does not clearly indicate independent
	 * instances. It indicates non-singleton instances, which may correspond to a scoped
	 * bean as well. Use the {@link #isPrototype} operation to explicitly check for
	...
	/**
	 * Is this bean a prototype? That is, will {@link #getBean} always return independent
	 * instances?
	 * <p>
	 * Note: This method returning {@code false} does not clearly indicate a singleton
	 * object. It indicates non-independent instances, which may correspond to a scoped
	 * bean as well. Use the {@link #isSingleton} operation to explicitly check for a
	...
	/**
	 * Check whether the bean with the given name matches the specified type. More
	 * specifically, check whether a {@link #getBean} call for the given name would return
	 * an object that is assignable to the specified target type.
	 * <p>
	 * Translates aliases back to the corresponding canonical bean name.
	 * <p>
	 * Will ask the parent factory if the bean cannot be found in this factory instance.
	 ...
	 * @return {@code true} if the bean type matches, {@code false} if it doesn't match or
	...
	/**
	 * Determine the type of the bean with the given name. More specifically, determine
	 ...
	 * as exposed by {@link FactoryBean#getObjectType()}. This may lead to the
	 * initialization of a previously uninitialized {@code FactoryBean} (see
	 ...
	 * {@code allowFactoryBeanInit} flag, this may lead to the initialization of a
	 * previously uninitialized {@code FactoryBean} if no early type information is
	 * available.
	 * <p>
	 * Translates aliases back to the corresponding canonical bean name.
	 * <p>
	 * Will ask the parent factory if the bean cannot be found in this factory instance.
	 * @param name the name of the bean to query
	 * @param allowFactoryBeanInit whether a {@code FactoryBean} may get initialized just
	 * for the purpose of determining its object type
	 * @return the type of the bean, or {@code null} if not determinable
	 * @throws NoSuchBeanDefinitionException if there is no bean with the given name
	 * @since 5.2
	 * @see #getBean
	 ...
	 * <p>
	 * If the given name is an alias, the corresponding original bean name and other
	 * aliases (if any) will be returned, with the original bean name being the first
