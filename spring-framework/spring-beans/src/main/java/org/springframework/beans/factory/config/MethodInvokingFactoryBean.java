 ...
 * method invocation. For most use cases it is better to just use the container's built-in
 * factory method support for the same purpose, since that is smarter at converting
 * arguments. This factory bean is still useful though when you need to call a method
 * which doesn't return any value (for example, a static class method to force some sort
 * of initialization to happen). This use case is not supported by factory methods, since
 * a return value is needed to obtain the bean instance.
 *
 * <p>
 * Note that as it is expected to be used mostly for accessing factory methods, this
 * factory by default operates in a <b>singleton</b> fashion. The first request to
 * {@link #getObject} by the owning bean factory will cause a method invocation, whose
 * return value will be cached for subsequent requests. An internal {@link #setSingleton
 * singleton} property may be set to "false", to cause this factory to invoke the target
 * method each time it is asked for an object.
 *
 * <p>
 * <b>NOTE: If your target method does not produce a result to expose, consider
 * {@link MethodInvokingBean} instead, which avoids the type determination and lifecycle
 * limitations that this {@link MethodInvokingFactoryBean} comes with.</b>
 *
 * <p>
 ...
 * this class to call a static factory method:
 *
 * <pre class="code">
...
public class MethodInvokingFactoryBean extends MethodInvokingBean implements FactoryBean<Object> {
	...
	/**
	 * Set if a singleton should be created, or a new object on each {@link #getObject()}
	 * request otherwise. Default is "true".
	 */
	...
	/**
	 * Returns the same value each time if the singleton property is set to "true",
	 * otherwise returns the value returned from invoking the specified method on the fly.
	 */
	@Override
	@Nullable
