 ...
 * implemented by bean factories that are capable of autowiring, provided that they want
 ...
 * stick to {@link org.springframework.beans.factory.BeanFactory} or
 ...
 * existing bean instances that Spring does not control the lifecycle of. This is
 * particularly useful for WebWork Actions and Tapestry Page objects, for example.
 *
 * <p>
 * Note that this interface is not implemented by
 * {@link org.springframework.context.ApplicationContext} facades, as it is hardly ever
 * used by application code. That said, it is available from an application context too,
 * accessible through ApplicationContext's
 * {@link org.springframework.context.ApplicationContext#getAutowireCapableBeanFactory()}
 * method.
 *
 * <p>
 * You may also implement the {@link org.springframework.beans.factory.BeanFactoryAware}
 * interface, which exposes the internal BeanFactory even when running in an
 * ApplicationContext, to get access to an AutowireCapableBeanFactory: simply cast the
 * passed-in BeanFactory to AutowireCapableBeanFactory.
 *
...
public interface AutowireCapableBeanFactory extends BeanFactory {
	...
	/**
	 * Suffix for the "original instance" convention when initializing an existing bean
	 * instance: to be appended to the fully-qualified bean class name, e.g.
	 * "com.mypackage.MyClass.ORIGINAL", in order to enforce the given instance to be
	 * returned, i.e. no proxies etc.
	 * @since 5.1
	 ...
	 * and methods as well as applying all standard bean initialization callbacks. It does
	 * <i>not</i> imply traditional by-name or by-type autowiring of properties; use
	 * {@link #createBean(Class, int, boolean)} for those purposes.
	 ...
	 * {@link #autowireBeanProperties} for those purposes.
	 ...
	 * in the bean instance (not applicable to autowiring a constructor, thus ignored
	 * there)
	 * @return the new bean instance
	 * @throws BeansException if instantiation or wiring failed
	 * @see #AUTOWIRE_NO
	 ...
	 * to {@link BeanPostProcessor BeanPostProcessors}; can follow the
	 * {@link #ORIGINAL_INSTANCE_SUFFIX} convention in order to enforce the given instance
	 ...
	 * <p>
	 * This is effectively a variant of {@link #getBean(Class)} which preserves the bean
