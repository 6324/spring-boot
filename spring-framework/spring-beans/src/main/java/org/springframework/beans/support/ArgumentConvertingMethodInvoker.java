...
/**
 * Subclass of {@link MethodInvoker} that tries to convert the given arguments for the
 * actual target method via a {@link TypeConverter}.
 *
 * <p>
 * Supports flexible argument conversions, in particular for invoking a specific
 * overloaded method.
 *
...
public class ArgumentConvertingMethodInvoker extends MethodInvoker {
	 ...
	 * <p>
	 * Default is a {@link org.springframework.beans.SimpleTypeConverter}. Can be
	 * overridden with any TypeConverter implementation, typically a pre-configured
	 ...
	 * <p>
	 * Can be cast to {@link org.springframework.beans.PropertyEditorRegistry} if direct
	 * access to the underlying PropertyEditors is desired (provided that the present
	 ...
	 * <p>
	 * Called if no explicit TypeConverter has been specified. The default implementation
	 * builds a {@link org.springframework.beans.SimpleTypeConverter}. Can be overridden
	 * in subclasses.
	 */
	...
	protected Method findMatchingMethod() {
		...
		// Second pass: look for method where arguments can be converted to parameter
		// types.
	...
	/**
	 * Actually find a method with matching parameter type, i.e. where each argument value
	 * is assignable to the corresponding parameter type.
				...
				if (candidate.getName().equals(targetMethod)) {
					...
					if (parameterCount == argCount) {
						...
						for (int j = 0; j < argCount && match; j++) {
							// Verify that the supplied argument is assignable to the
							// method parameter.
							try {
