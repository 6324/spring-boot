 ...
 *
 * <p>
 * Records intermediate compilation state as the bytecode is generated. Also includes
...
public class CodeFlow implements Opcodes {
	...
	/**
	 * Name of the class being generated. Typically used when generating code that
	 * accesses freshly generated fields on the generated type.
	 */
	...
	/**
	 * As SpEL ast nodes are called to generate code for the main evaluation method they
	 * can register to add a field to this class. Any registered FieldAdders will be
	 * called after the main evaluation function has finished being generated.
	 */
	@Nullable
	...
	/**
	 * As SpEL ast nodes are called to generate code for the main evaluation method they
	 * can register to add code to a static initializer in the class. Any registered
	 * ClinitAdders will be called after the main evaluation function has finished being
	 * generated.
	 */
	@Nullable
	...
	/**
	 * When code generation requires holding a value in a class level field, this is used
	 * to track the next available field id (used as a name suffix).
	 */
	private int nextFieldId = 1;

	/**
	 * When code generation requires an intermediate variable within a method, this method
	 * records the next available variable (variable 0 is 'this').
	 */
	...
	/**
	 * Push the bytecode to load the EvaluationContext (the second parameter passed to the
	 * compiled expression method).
	 * @param mv the visitor into which the load instruction should be inserted
	 * @since 4.3.4
	 */
	...
	/**
	 * Called after the main expression evaluation method has been generated, this method
	 * will callback any registered FieldAdders or ClinitAdders to add any extra
	 * information to the class representing the compiled expression.
	 */
	public void finish() {
	...
	/**
	 * Register a FieldAdder which will add a new field to the generated class to support
	 * the code produced by an ast nodes primary generateCode() method.
	 */
	...
	/**
	 * Register a ClinitAdder which will add code to the static initializer in the
	 * generated class to support the code produced by an ast nodes primary generateCode()
	 * method.
	 */
	...
	/**
	 * For numbers, use the appropriate method on the number to convert it to the
	 * primitive type requested.
	 * @param mv the method visitor into which instructions should be inserted
	 * @param targetDescriptor the primitive type desired as output
	 * @param stackDescriptor the descriptor of the type on top of the stack
	 */
	public static void insertUnboxNumberInsns(MethodVisitor mv, char targetDescriptor,
			@Nullable String stackDescriptor) {
	...
	/**
	 * Insert any necessary numeric conversion bytecodes based upon what is on the stack
	 * and the desired target type.
	 ...
	 */
	public static void insertAnyNecessaryTypeConversionBytecodes(MethodVisitor mv, char targetDescriptor,
	 ...
	 * for the method parameters surrounded with parentheses, followed by the descriptor
	 * for the return type. Note the descriptors here are JVM descriptors, unlike the
	 * other descriptor forms the compiler is using which do not include the trailing
	 * semicolon.
	 ...
	 * the descriptor for the return type, which is always "V". Note the descriptors here
	 * are JVM descriptors, unlike the other descriptor forms the compiler is using which
	 * do not include the trailing semicolon.
	...
	/**
	 * Determine whether the descriptor is for a boolean primitive or boolean reference
	 * type.
	 ...
	 * least one of the types is in boxed form (i.e. single char descriptor).
	 * @return {@code true} if it is possible to get (via boxing) from one descriptor to
	 * the other
	 */
	...
	/**
	 * Determine whether the given number is to be considered as an integer for the
	 * purposes of a numeric operation at the bytecode level.
	...
	/**
	 * Produce appropriate bytecode to store a stack item in an array. The instruction to
	 * use varies depending on whether the type is a primitive or reference type.
	 * @param mv where to insert the bytecode
	...
	/**
	 * Produce the correct bytecode to build an array. The opcode to use and the signature
	 * to pass along with the opcode can vary depending on the signature of the array
	 * type.
	 ...
	 */
	public static void insertNumericUnboxOrPrimitiveTypeCoercion(MethodVisitor mv, @Nullable String stackDescriptor,
