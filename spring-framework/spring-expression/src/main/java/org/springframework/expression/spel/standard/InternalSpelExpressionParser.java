...
class InternalSpelExpressionParser extends TemplateAwareExpressionParser {
	...
	// unaryExpression: (PLUS^ | MINUS^ | BANG^ | INC^ | DEC^) unaryExpression |
	// primaryExpression ;
	@Nullable
	...
	private boolean maybeEatFunctionOrVar() {
		...
		if (args == null) {
			push(new VariableReference(functionOrVariableName.stringValue(), t.startPos,
					functionOrVariableName.endPos));
			return true;
		}

		push(new FunctionReference(functionOrVariableName.stringValue(), t.startPos, functionOrVariableName.endPos,
				args));
		return true;
	}
	...
	private void eatConstructorArgs(List<SpelNodeImpl> accumulatedArguments) {
		if (!peekToken(TokenKind.LPAREN)) {
			throw new InternalParseException(new SpelParseException(this.expressionString, positionOf(peekToken()),
	...
	/**
	 * Eat an identifier, possibly qualified (meaning that it is dotted). TODO AndyC Could
	 * create complete identifiers (a.b.c) here rather than a sequence of them? (a, b, c)
	 */
	private SpelNodeImpl eatPossiblyQualifiedId() {
		...
		if (qualifiedIdPieces.isEmpty()) {
			...
			throw internalException(node.startPos, SpelMessage.NOT_EXPECTED_TOKEN, "qualified ID",
	...
	private boolean maybeEatMethodOrProperty(boolean nullSafeNavigation) {
		if (peekToken(TokenKind.IDENTIFIER)) {
			...
			// TODO what is the end position for a method reference? the name or the last
			// arg?
			return true;
		}
		return false;
	...
	// constructor
	// : ('new' qualifiedId LPAREN) => 'new' qualifiedId ctorArgs -> ^(CONSTRUCTOR
	...
	private Token eatToken(TokenKind expectedKind) {
		...
		if (t.kind != expectedKind) {
			throw internalException(t.startPos, SpelMessage.NOT_EXPECTED_TOKEN, expectedKind.toString().toLowerCase(),
	...
	private boolean peekToken(TokenKind desiredTokenKind, boolean consumeIfMatched) {
		...
		if (desiredTokenKind == TokenKind.IDENTIFIER) {
			...
			// in which case we can treat it as an identifier. The list is represented
			// here:
			// Tokenizer.alternativeOperatorNames and those ones are in order in the
			// TokenKind enum.
			if (t.kind.ordinal() >= TokenKind.DIV.ordinal() && t.kind.ordinal() <= TokenKind.NOT.ordinal()
					&& t.data != null) {
				// if t.data were null, we'd know it wasn't the textual form, it was the
				// symbol form
				return true;
			}
		}
