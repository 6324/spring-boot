 ...
 *
 * <p>
 * This interface places no restrictions, and so implementors are free to access
 * properties directly as fields or through getters or in any other way they see as
 * appropriate.
 *
 * <p>
 ...
 * it will be called for all property references and given a chance to determine if it can
 * read or write them.
 *
 * <p>
 * Property resolvers are considered to be ordered, and each will be called in turn. The
 * only rule that affects the call order is that any resolver naming the target class
 * directly in {@link #getSpecificTargetClasses()} will be called first, before the
 * general resolvers.
 *
 * @author Andy Clement
 * @since 3.0
...
public interface PropertyAccessor {
	 ...
	 * <p>
	 * Returning {@code null} indicates this is a general resolver that can be called in
	 * an attempt to resolve a property on any type.
	 * @return an array of classes that this resolver is suitable for (or {@code null} if
	 * a general resolver)
	 */
	@Nullable
	 ...
	 * {@link #canRead} also returns {@code true}.
	 * @param context the evaluation context in which the access is being attempted
	 * @param target the target object upon which the property is being accessed
	 * @param name the name of the property being accessed
	 * @return a TypedValue object wrapping the property value read and a type descriptor
	 * for it
	 ...
	 * {@link #canWrite} also returns {@code true}.
	 * @param context the evaluation context in which the access is being attempted
	 * @param target the target object upon which the property is being accessed
	 * @param name the name of the property being accessed
