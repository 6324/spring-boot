...
public class HttpComponentsClientHttpRequestFactory implements ClientHttpRequestFactory, DisposableBean {
	 ...
	 * default {@link HttpClient} based on system properties.
	 */
	...
	/**
	 * Set the {@code HttpClient} used for {@linkplain #createRequest(URI, HttpMethod)
	...
	/**
	 * Set the connection timeout for the underlying {@link RequestConfig}. A timeout
	 ...
	 * <p>
	 * This options does not affect connection timeouts for SSL handshakes or CONNECT
	 * requests; for that, it is required to use the
	 * {@link org.apache.http.config.SocketConfig} on the {@link HttpClient} itself.
	 ...
	 * connection manager using the underlying {@link RequestConfig}. A timeout value of 0
	...
	/**
	 * Set the socket read timeout for the underlying {@link RequestConfig}. A timeout
	 * value of 0 specifies an infinite timeout.
	 * <p>
	 * Additional properties can be configured by specifying a {@link RequestConfig}
	 * instance on a custom {@link HttpClient}.
	 ...
	 * <p>
	 * This may be useful for example in mutual TLS authentication where a different
	 * {@code RestTemplate} for each client certificate such that all calls made through a
	 * given {@code RestTemplate} instance as associated for the same client identity.
	 * {@link HttpClientContext#setUserToken(Object)} can be used to specify a fixed user
	...
	/**
	 * Create a default {@link RequestConfig} to use with the given client. Can return
	 * {@code null} to indicate that no custom request config should be set and the
	 * defaults of the {@link HttpClient} should be used.
	 * <p>
	...
	/**
	 * Merge the given {@link HttpClient}-level {@link RequestConfig} with the
	 * factory-level {@link RequestConfig}, if necessary.
	...
	/**
	 * An alternative to {@link org.apache.http.client.methods.HttpDelete} that extends
	 * {@link org.apache.http.client.methods.HttpEntityEnclosingRequestBase} rather than
	 * {@link org.apache.http.client.methods.HttpRequestBase} and hence allows HTTP delete
	 * with a request body. For use with the RestTemplate exchange methods which allow the
	 * combination of HTTP DELETE with an entity.
	 *
	 * @since 4.1.2
	 */
