 ...
 * through annotating corresponding parameters of an
 * {@link RequestMapping @RequestMapping} method.
 *
 * <p>
 * Can also be used to expose reference data to a web view through annotating accessor
 * methods in a controller class with {@link RequestMapping @RequestMapping} methods. Such
 * accessor methods are allowed to have any arguments that
 * {@link RequestMapping @RequestMapping} methods support, returning the model attribute
 * value to expose.
 *
 * <p>
 * Note however that reference data and all other model content is not available to web
