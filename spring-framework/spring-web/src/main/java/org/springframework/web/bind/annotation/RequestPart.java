 ...
 * analogous to what @{@link RequestBody} does to resolve an argument based on the content
 * of a non-multipart regular request.
 *
 * <p>
 * Note that @{@link RequestParam} annotation can also be used to associate the part of a
 * "multipart/form-data" request with a method argument supporting the same method
 * argument types. The main difference is that when the method argument is not a String or
 * raw {@code MultipartFile} / {@code Part}, {@code @RequestParam} relies on type
 * conversion via a registered {@link Converter} or {@link PropertyEditor} while
 * {@link RequestPart} relies on {@link HttpMessageConverter HttpMessageConverters} taking
 * into consideration the 'Content-Type' header of the request part. {@link RequestParam}
 * is likely to be used with name-value form fields while {@link RequestPart} is likely to
