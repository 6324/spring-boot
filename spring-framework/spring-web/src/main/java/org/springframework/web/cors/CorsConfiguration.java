...
/**
 * A container for CORS configuration along with methods to check against the actual
 * origin, HTTP methods, and headers of a given request.
 *
 * <p>
 * By default a newly created {@code CorsConfiguration} does not permit any cross-origin
 * requests and must be configured explicitly to indicate what should be allowed. Use
 * {@link #applyPermitDefaultValues()} to flip the initialization model to start with open
...
public class CorsConfiguration {
	...
	private static final List<HttpMethod> DEFAULT_METHODS = Collections
			.unmodifiableList(Arrays.asList(HttpMethod.GET, HttpMethod.HEAD));

	private static final List<String> DEFAULT_PERMIT_METHODS = Collections
			.unmodifiableList(Arrays.asList(HttpMethod.GET.name(), HttpMethod.HEAD.name(), HttpMethod.POST.name()));
	...
	/**
	 * Construct a new {@code CorsConfiguration} instance with no cross-origin requests
	 * allowed for any origin by default.
	...
	/**
	 * Construct a new {@code CorsConfiguration} instance by copying all values from the
	 * supplied {@code CorsConfiguration}.
	 */
	...
	/**
	 * Set the HTTP methods to allow, e.g. {@code "GET"}, {@code "POST"}, {@code "PUT"},
	 * etc.
	 * <p>
	 ...
	 * reflect the client-originated address. Consider using the
	...
	/**
	 * Return the allowed HTTP methods, or {@code null} in which case only {@code "GET"}
	 * and {@code "HEAD"} allowed.
	 ...
	 * <p>
	 * A header name is not required to be listed if it is one of: {@code Cache-Control},
	 * {@code Content-Language}, {@code Expires}, {@code Last-Modified}, or
	 * {@code Pragma}.
	 * <p>
	 * By default this is not set.
	 */
	 ...
	 * {@code Expires}, {@code Last-Modified}, or {@code Pragma}) that an actual response
	 * might have and can be exposed.
	 * <p>
	 * The special value {@code "*"} allows all headers to be exposed for non-credentialed
	 * requests.
	 * <p>
	 * By default this is not set.
	 */
	...
	/**
	 * Configure how long, as a duration, the response from a pre-flight request can be
	 * cached by clients.
	 * @since 5.2
	 * @see #setMaxAge(Long)
	 */
	 ...
	 * cross-origin requests and must be configured explicitly to indicate what should be
	 * allowed.
	 * <p>
	 * Use this method to flip the initialization model to start with open defaults that
	 * permit all cross-origin requests for GET, HEAD, and POST requests. Note however
	 * that this method will not override any existing values already set.
	 * <p>
	...
	public CorsConfiguration applyPermitDefaultValues() {
		...
		if (this.allowedMethods == null) {
			...
			this.resolvedMethods = DEFAULT_PERMIT_METHODS.stream().map(HttpMethod::resolve)
	...
	/**
	 * Combine the non-null properties of the supplied {@code CorsConfiguration} with this
	 * one.
	 * <p>
	 * When combining single values like {@code allowCredentials} or {@code maxAge},
	 * {@code this} properties are overridden by non-null {@code other} properties if any.
	 * <p>
	 * Combining lists like {@code allowedOrigins}, {@code allowedMethods},
	 * {@code allowedHeaders} or {@code exposedHeaders} is done in an additive way. For
	 * example, combining {@code ["GET", "POST"]} with {@code ["PATCH"]} results in
	 * {@code ["GET", "POST", "PATCH"]}, but keep in mind that combining
	 * {@code ["GET", "POST"]} with {@code ["*"]} results in {@code ["*"]}.
	 * <p>
	 * Notice that default permit values set by
	 * {@link CorsConfiguration#applyPermitDefaultValues()} are overridden by any value
	 * explicitly defined.
	 * @return the combined {@code CorsConfiguration}, or {@code this} configuration if
	 * the supplied configuration is {@code null}
	 */
	@Nullable
	 ...
	 * @return the origin to use for the response, or {@code null} which means the request
	 ...
	 * {@code Access-Control-Request-Method} header on a pre-flight request) against the
	 * configured allowed methods.
	 * @param requestMethod the HTTP request method to check
	 * @return the list of HTTP methods to list in the response of a pre-flight request,
	 * or {@code null} if the supplied {@code requestMethod} is not allowed
	 */
	@Nullable
