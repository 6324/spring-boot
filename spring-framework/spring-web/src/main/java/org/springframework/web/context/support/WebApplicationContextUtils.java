...
/**
 * Convenience methods for retrieving the root {@link WebApplicationContext} for a given
 * {@link ServletContext}. This is useful for programmatically accessing a Spring
 * application context from within custom web views or MVC actions.
 *
 * <p>
 * Note that there are more convenient ways of accessing the root context for many web
 * frameworks, either part of Spring or available as an external library. This helper
 * class is just the most generic way to access the root context.
 *
...
public abstract class WebApplicationContextUtils {
	...
	/**
	 * Find the root {@code WebApplicationContext} for this web app, typically loaded via
	 * {@link org.springframework.web.context.ContextLoaderListener}.
	 * <p>
	 * Will rethrow an exception that happened on root context startup, to differentiate
	 * between a failed context startup and no context at all.
	 ...
	 * {@link #initServletPropertySources(MutablePropertySources, ServletContext, ServletConfig)}
	 * that always provides {@code null} for the {@link ServletConfig} parameter.
	 * @see #initServletPropertySources(MutablePropertySources, ServletContext,
	 * ServletConfig)
	 */
	 ...
	 * <p>
	 * This method is idempotent with respect to the fact it may be called any number of
	 * times but will perform replacement of stub property sources with their
	 * corresponding actual property sources once and only once.
	 * @param sources the {@link MutablePropertySources} to initialize (must not be
	 * {@code null})
	 ...
	 * @param servletConfig the current {@link ServletConfig} (ignored if {@code null} or
	 * if the {@link StandardServletEnvironment#SERVLET_CONFIG_PROPERTY_SOURCE_NAME
