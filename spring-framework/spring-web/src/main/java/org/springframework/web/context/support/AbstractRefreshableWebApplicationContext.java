 ...
 * {@link org.springframework.web.context.ConfigurableWebApplicationContext} interface for
 * web environments. Provides a "configLocations" property, to be populated through the
 * ConfigurableWebApplicationContext interface on web application startup.
 *
 * <p>
 * This class is as easy to subclass as AbstractRefreshableApplicationContext: All you
 * need to implements is the {@link #loadBeanDefinitions} method; see the superclass
 * javadoc for details. Note that implementations are supposed to load bean definitions
 * from the files specified by the locations returned by the {@link #getConfigLocations}
 * method.
 *
 * <p>
 ...
 * Such a context implementation can be specified as "contextClass" context-param for
 * {@link org.springframework.web.context.ContextLoader} or as "contextClass" init-param
 * for {@link org.springframework.web.servlet.FrameworkServlet}, replacing the default
 * {@link XmlWebApplicationContext}. It will then automatically receive the
 * "contextConfigLocation" context-param or init-param, respectively.
 *
 * <p>
 * Note that WebApplicationContext implementations are generally supposed to configure
 * themselves based on the configuration received through the
 * {@link ConfigurableWebApplicationContext} interface. In contrast, a standalone
 * application context might allow for configuration in custom startup code (for example,
