 ...
 * hidden form field ({@code _method}) to pass the "real" HTTP method along. This filter
 * reads that parameter and changes the {@link ServerHttpRequest#getMethod()} return value
 * using {@link ServerWebExchange#mutate()}.
 *
 * <p>
 * The name of the request parameter defaults to {@code _method}, but can be adapted via
 * the {@link #setMethodParamName(String) methodParamName} property.
 *
...
public class HiddenHttpMethodFilter implements WebFilter {
	...
	private static final List<HttpMethod> ALLOWED_METHODS = Collections
			.unmodifiableList(Arrays.asList(HttpMethod.PUT, HttpMethod.DELETE, HttpMethod.PATCH));
