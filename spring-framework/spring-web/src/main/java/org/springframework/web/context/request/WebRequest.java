...
/**
 * Generic interface for a web request. Mainly intended for generic web request
 * interceptors, giving them access to general request metadata, not for actual handling
 * of the request.
 *
...
public interface WebRequest extends RequestAttributes {
	...
	/**
	 * Return the request header values for the given header name, or {@code null} if
	 * none.
	 * <p>
	...
	/**
	 * Return the request parameter values for the given parameter name, or {@code null}
	 * if none.
	 * <p>
	...
	/**
	 * Return the context path for this request (usually the base path that the current
	 * web application is mapped to).
	...
	/**
	 * Return whether this request has been sent over a secure transport mechanism (such
	 * as SSL).
	 ...
	 * <p>
	 * This will also transparently set the "Last-Modified" response header and HTTP
	 ...
	 * <p>
	 * <strong>Note:</strong> you can use either this {@code #checkNotModified(long)}
	 ...
	 * <p>
	 * If the "If-Modified-Since" header is set but cannot be parsed to a date value, this
	 * method will ignore the header and proceed with setting the last-modified timestamp
	 * on the response.
	 * @param lastModifiedTimestamp the last-modified timestamp in milliseconds that the
	 * application determined for the underlying resource
	 * @return whether the request qualifies as not modified, allowing to abort request
	 * processing and relying on the response telling the client that the content has not
	 * been modified
	 */
	 ...
	 * <p>
	 * This will also transparently set the "ETag" response header and HTTP status when
	 * applicable.
	 * <p>
	 * Typical usage: <pre class="code">
	 * public String myHandleMethod(WebRequest request, Model model) {
	 ...
	 * }</pre>
	 * <p>
	 * <strong>Note:</strong> you can use either this {@code #checkNotModified(String)}
	 * method; or {@link #checkNotModified(long)}. If you want enforce both a strong
	 * entity tag and a Last-Modified value, as recommended by the HTTP specification,
	 * then you should use {@link #checkNotModified(String, long)}.
	 ...
	 * application.
	 * <p>
	 * This will also transparently set the "ETag" and "Last-Modified" response headers,
	...
	/**
	 * Get a short description of this request, typically containing request URI and
	 * session id.
	 * @param includeClientInfo whether to include client-specific information such as
