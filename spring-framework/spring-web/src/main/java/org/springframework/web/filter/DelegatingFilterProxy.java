 ...
 * implements the Filter interface. Supports a "targetBeanName" filter init-param in
 * {@code web.xml}, specifying the name of the target bean in the Spring application
 * context.
 *
 * <p>
 * {@code web.xml} will usually contain a {@code DelegatingFilterProxy} definition, with
 * the specified {@code filter-name} corresponding to a bean name in Spring's root
 * application context. All calls to the filter proxy will then be delegated to that bean
 * in the Spring context, which is required to implement the standard Servlet Filter
 * interface.
 *
 * <p>
 * This approach is particularly useful for Filter implementation with complex setup
 * needs, allowing to apply the full Spring bean definition machinery to Filter instances.
 * Alternatively, consider standard Filter setup in combination with looking up service
 * beans from the Spring root application context.
 *
 * <p>
 * <b>NOTE:</b> The lifecycle methods defined by the Servlet Filter interface will by
 * default <i>not</i> be delegated to the target bean, relying on the Spring application
 * context to manage the lifecycle of that bean. Specifying the "targetFilterLifecycle"
 * filter init-param as "true" will enforce invocation of the {@code Filter.init} and
 * {@code Filter.destroy} lifecycle methods on the target bean, letting the servlet
...
public class DelegatingFilterProxy extends GenericFilterBean {
	 ...
	 * <p>
	 * For use in Servlet 3.0+ environments where instance-based registration of filters
	 * is supported.
	...
	/**
	 * Create a new {@code DelegatingFilterProxy} that will retrieve the named target bean
	 * from the Spring {@code WebApplicationContext} found in the {@code ServletContext}
	...
	/**
	 * Create a new {@code DelegatingFilterProxy} that will retrieve the named target bean
	 * from the given Spring {@code WebApplicationContext}.
	 * <p>
	 * For use in Servlet 3.0+ environments where instance-based registration of filters
	 * is supported.
	 * <p>
	 ...
	 * If it has not, and if the context implements
	 * {@link ConfigurableApplicationContext}, a
	 * {@link ConfigurableApplicationContext#refresh() refresh()} will be attempted before
	 ...
	 * the {@link WebApplicationContext} from which to load the delegate {@link Filter}
	 * bean.
	 */
	@Nullable
	 ...
	 * must implement the standard Servlet Filter interface.
	 * <p>
	 * By default, the {@code filter-name} as specified for the DelegatingFilterProxy in
	...
	/**
	 * Set whether to invoke the {@code Filter.init} and {@code Filter.destroy} lifecycle
	 * methods on the target bean.
	 * <p>
	 ...
	 * container will control the lifecycle of the target Filter, with this proxy
	...
	protected WebApplicationContext findWebApplicationContext() {
		if (this.webApplicationContext != null) {
			...
			if (this.webApplicationContext instanceof ConfigurableApplicationContext) {
				...
				if (!cac.isActive()) {
					// The context has not yet been refreshed -> do so before returning
					// it...
					cac.refresh();
				}
			}
	 ...
	 * the standard {@code Filter.init} method on it, passing in the FilterConfig of this
	 * Filter proxy.
